#!/usr/bin/env python3
# Standalone replay for 20250921_0007; this file intentionally contains the full BGPy replay engine.
# Event type: recovery; prefix: 204.92.234.0/23; root AS: 32613.
# Evidence summary: The prefix 204.92.234.0/23 was being announced by AS 14843 before the event. After the event, the legitimate owner AS 32613 began announcing the prefix. The observed post-event origin AS (32613) equals the legitimate owner AS (32613), so this event is classified as 'other' — likely a correction where the legitimate owner reclaimed the prefix after AS 14843 was previously announcing it.
...
* Origin recovery: replay the post-withdrawal state in which the legitimate
  ...
  seeded.  For this event, historical intermediate routes can be warm-started
  from a receiver-disjoint calibration partition; the held-out receivers are
  never seeded and remain the evaluation set.
...
CalibrationRouteAnchorMode = Literal["none", "intermediate"]
...
def derive_calibration_route_anchors(
    config: EventConfig,
    resolved: dict[str, Any],
) -> list[dict[str, Any]]:
    """Derive deterministic intermediate-route anchors from calibration only.

    The observed propagation paths are stored root-to-receiver, whereas BGPy's
    Announcement.as_path is stored receiver-to-origin.  For every intermediate
    AS, this function therefore reverses the calibrated prefix of the path.
    The anomaly root and the final calibration receiver are deliberately
    excluded: the root already originates the route, and seeding a receiver
    would make receiver-holdout evaluation less meaningful.
    """

    candidate_counts: dict[
        tuple[str, int], Counter[tuple[int, ...]]
    ] = {}
    candidate_sources: dict[tuple[str, int], set[int]] = {}
    for item in resolved.get("calibration_paths", []):
        prefix = str(item["prefix"])
        propagation_path = tuple(
            int(asn)
            for asn in item.get(
                "propagation_path_normalized", item["propagation_path"]
            )
        )
        if len(propagation_path) < 3 or propagation_path[0] != config.root_asn:
            continue
        calibration_receiver = int(item["receiver_as"])
        # Index 0 is the root and the last index is the held-in calibration
        # receiver.  Only the shared/intermediate route state is anchored.
        for index in range(1, len(propagation_path) - 1):
            anchor_asn = propagation_path[index]
            as_path_view = tuple(reversed(propagation_path[: index + 1]))
            key = (prefix, anchor_asn)
            candidate_counts.setdefault(key, Counter())[as_path_view] += 1
            candidate_sources.setdefault(key, set()).add(calibration_receiver)

    anchors: list[dict[str, Any]] = []
    for (prefix, anchor_asn), counts in sorted(candidate_counts.items()):
        # Highest frequency wins; lexical order makes ties reproducible.
        selected_path = min(counts, key=lambda path: (-counts[path], path))
        anchors.append(
            {
                "prefix": prefix,
                "anchor_as": anchor_asn,
                "as_path_view": list(selected_path),
                "previous_hop_as": int(selected_path[1]),
                "selected_candidate_count": counts[selected_path],
                "calibration_receiver_count": len(
                    candidate_sources[(prefix, anchor_asn)]
                ),
                "candidate_paths": [
                    {"as_path_view": list(path), "count": counts[path]}
                    for path in sorted(counts, key=lambda path: (-counts[path], path))
                ],
...
def build_announcements(
    ...
    *,
    ...
    calibration_route_anchor_mode: CalibrationRouteAnchorMode = "intermediate",
    ...
    audit: dict[str, Any] = {
        "announcements": [],
        "warnings": [],
        "calibration_route_anchor_mode": calibration_route_anchor_mode,
        "calibration_route_anchor_count": 0,
        "calibration_route_anchor_source": "calibration_partition_only",
    }
    ...
    if calibration_route_anchor_mode not in {"none", "intermediate"}:
        raise ValueError(
            "Unsupported calibration route anchor mode: "
            f"{calibration_route_anchor_mode}"
        )
    ...
    if config.anomaly_type == "hijack":
        ...
        if calibration_route_anchor_mode == "intermediate":
            for details in derive_calibration_route_anchors(config, resolved):
                anchor_asn = int(details["anchor_as"])
                previous_hop_asn = int(details["previous_hop_as"])
                if anchor_asn not in as_graph.as_dict:
                    raise RuntimeError(
                        f"Calibration route anchor AS{anchor_asn} is absent "
                        "from the selected CAIDA graph"
                    )
                anchor_obj = as_graph.as_dict[anchor_asn]
                observed_relationship = relationship_name(
                    anchor_obj, previous_hop_asn
                )
                recv_relationship = relationship_enum(
                    anchor_obj, previous_hop_asn
                )
                relationship_source = "event_overlay_or_caida"
                if recv_relationship is None:
                    recv_relationship = Relationships.PEERS
                    relationship_source = "explicit_fallback_peer"
                    audit["warnings"].append(
                        f"AS{previous_hop_asn} is not adjacent to calibration "
                        f"anchor AS{anchor_asn} for {details['prefix']}; "
                        "recv_relationship defaults to peer."
                    )

                as_path_view = tuple(
                    int(asn) for asn in details["as_path_view"]
                )
                anchored = Announcement(
                    prefix=details["prefix"],
                    as_path=as_path_view,
                    seed_asn=anchor_asn,
                    next_hop_asn=anchor_asn,
                ...
                audit["announcements"].append(
                    {
                        **details,
                        "seed_as": anchor_asn,
                        "type": "calibration_route_anchor",
                        "received_relationship": (
                            "peer"
                            if relationship_source == "explicit_fallback_peer"
                            else observed_relationship
                        ),
                        "relationship_source": relationship_source,
                        "calibration_note": (
                            "Historical intermediate route selected from the "
                            "disjoint calibration partition; no evaluation "
                            "receiver path was used."
                        ),
                    }
                )
            audit["calibration_route_anchor_count"] = sum(
                item.get("type") == "calibration_route_anchor"
                for item in audit["announcements"]
            )
    else:
...
def run_simulation(
    ...
    *,
    ...
    calibration_route_anchor_mode: CalibrationRouteAnchorMode,
...
def compare_with_observed_paths(
    ...
    calibration_route_anchors_used = bool(
        simulation.get("calibration_route_anchor_count", 0)
    )
    calibration_route_anchor_overlap = (
        calibration_route_anchors_used
        and split_audit.get("calibration_evaluation_overlap", True)
    )
    ...
    if calibration_route_anchors_used:
        quality_flags.append("calibration_route_anchors_used")
    if calibration_route_anchor_overlap:
        quality_flags.append("evaluation_paths_used_for_route_anchors")
    ...
    return {
        ...
        "evaluation_quality": {
            ...
            "calibration_route_anchors_used": calibration_route_anchors_used,
            "calibration_route_anchor_count": int(
                simulation.get("calibration_route_anchor_count", 0)
            ),
            "calibration_route_anchor_source": simulation.get(
                "calibration_route_anchor_source"
            ),
            "calibration_route_anchor_evaluation_overlap": (
                calibration_route_anchor_overlap
            ),
            "evaluation_scope": (
                ...
                if calibration_route_anchor_overlap
                    ...
                    "receiver_holdout_with_calibration_route_anchors"
                    if calibration_route_anchors_used
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--calibration-route-anchor-mode",
        choices=("none", "intermediate"),
        default="intermediate",
        help=(
            "For recovery events, seed historical intermediate route state "
            "derived only from the calibration receiver partition. Use none "
            "for the uncalibrated CAIDA baseline. Default: intermediate."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def run_cli(config: EventConfig) -> None:
    ...
    engine, _scenario, announcement_audit = run_simulation(
        ...
        calibration_route_anchor_mode=args.calibration_route_anchor_mode,
    )
    ...
    simulation["calibration_route_anchor_mode"] = (
        args.calibration_route_anchor_mode
    )
    simulation["calibration_route_anchor_count"] = int(
        announcement_audit.get("calibration_route_anchor_count", 0)
    )
    simulation["calibration_route_anchor_source"] = (
        announcement_audit.get("calibration_route_anchor_source")
    )
    ...
    complete_result = {
        "metadata": {
            **resolved["metadata"],
            ...
            "legal_route_mode": args.legal_route_mode,
            "path_source": args.path_source,
            "calibration_route_anchor_mode": (
                args.calibration_route_anchor_mode
            ),
            "calibration_route_anchor_count": int(
                announcement_audit.get("calibration_route_anchor_count", 0)
            ),
            "evaluation_scope": comparison["evaluation_quality"][
                "evaluation_scope"
            ],
        },
    ...
    print(
        "[+] Calibration route anchors: "
        f"{announcement_audit.get('calibration_route_anchor_count', 0)} "
        f"(mode={args.calibration_route_anchor_mode})"
    )
...
CONFIG = EventConfig(
    event_id='20250921_0007',
    event_name='Recovery event 20250921_0007',
    anomaly_type='recovery',
    canonical_prefix='204.92.234.0/23',
    root_asn=32613,
    victim_asn=14843,
    caida_snapshot=datetime(2025, 9, 1),
)
