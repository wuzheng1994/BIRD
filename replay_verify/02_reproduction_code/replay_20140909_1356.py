#!/usr/bin/env python3
# Standalone replay for 20140909_1356; this file intentionally contains the full BGPy replay engine.
# Event type: hijack; prefix: 95.128.184.0/22; root AS: 1239.
...
# AS1239 route.  CUSTOMER is a calibrated local-preference class, not a claim
# about the literal commercial relationship between every pair.
EVENT_ATTACK_FIRST_HOPS = (1299, 7018, 3257, 6453, 6830, 2497)
...
# AS1239, each key AS may export only to the listed next-hop ASNs.  This stops
# independently seeded first-hop anchors from contaminating another observed
# branch (for example AS3257 displacing AS1299 at AS3333/AS6881).  The mapping
# does not affect announcements originated by any other AS.
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    1239: frozenset(EVENT_ATTACK_FIRST_HOPS),
    1299: frozenset({3333, 15685, 50304}),
    7018: frozenset(),
    3257: frozenset({48039}),
    48039: frozenset({57821}),
    6453: frozenset({22652}),
    6830: frozenset({8758}),
        ...
        for prefix in resolved["anomalous_prefixes"]:
            ...
            for first_hop_asn in EVENT_ATTACK_FIRST_HOPS:
                ...
                anchored = Announcement(
                    ...
                    # first hop selected AS1239 during the incident.
                ...
                audit["announcements"].append(
                    {
                        ...
                        "calibration_note": (
                            ...
                            "the AS1239 route; not an independent prediction."
                        ),
                    }
                )
    elif config.anomaly_type == "recovery":
...
def make_event_export_policy(
    *,
    ...
    class EventConstrainedExportPolicy(BGP):
        name = "20140909 Event-Constrained Export BGP"
...
def validate_topology(
    ...
    observed_tree_edges = directed_edges(
        item["propagation_path"] for item in resolved["observed_paths"]
    )
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "primary experiment reconstructs the observed AS1239 propagation "
            "envelope rather than an artificial fully converged dual-origin state."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def run_cli(config: EventConfig) -> None:
    ...
    replay_state_model = (
        "event_anomaly_propagation_envelope"
        if args.legal_route_mode == "withdrawn"
        else "converged_dual_origin_competition"
    )
    comparison["evaluation_quality"]["replay_state_model"] = replay_state_model
    comparison["evaluation_quality"]["state_model_note"] = (
        "The primary envelope replay asks whether the observed AS1239 tree can "
        "propagate under the audited event constraints. Use "
        "--legal-route-mode after-observed for a separate converged dual-origin "
        "competition experiment; that experiment is not a reconstruction of "
        "the transient collector envelope."
    )
...
CONFIG = EventConfig(
    event_id='20140909_1356',
    event_name='Hijack event 20140909_1356',
    anomaly_type='hijack',
    canonical_prefix='95.128.184.0/22',
    root_asn=1239,
    victim_asn=34547,
