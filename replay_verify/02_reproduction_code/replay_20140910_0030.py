#!/usr/bin/env python3
# Standalone replay for 20140910_0030; this file intentionally contains the full BGPy replay engine.
# Event type: hijack; prefix: 177.200.2.0/23; root AS: 57807.
# Evidence summary: four event updates contain the same AS57807-to-AS6881 path.
...
# AS57807 route.  CUSTOMER is a calibrated local-preference class, not a claim
# about the literal commercial relationship between every pair.
EVENT_ATTACK_FIRST_HOPS = (44514,)
...
# AS57807, each key AS may export only to the listed next-hop ASNs.  This
# reproduces the single stable branch found in all four anomalous updates. The
# mapping does not affect announcements originated by any other AS.
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    57807: frozenset(EVENT_ATTACK_FIRST_HOPS),
    44514: frozenset({13110}),
    13110: frozenset({6939}),
    6939: frozenset({15685}),
        ...
        for prefix in resolved["anomalous_prefixes"]:
            ...
            for first_hop_asn in EVENT_ATTACK_FIRST_HOPS:
                ...
                anchored = Announcement(
                    ...
                    # first hop selected AS57807 during the incident.
                ...
                audit["announcements"].append(
                    {
                        ...
                        "calibration_note": (
                            ...
                            "the AS57807 route; not an independent prediction."
                        ),
                    }
                )
    elif config.anomaly_type == "recovery":
...
def make_event_export_policy(
    *,
    ...
    class EventConstrainedExportPolicy(BGP):
        name = "20140910 Event-Constrained Export BGP"
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "primary experiment reconstructs the observed AS57807 propagation "
...
def run_cli(config: EventConfig) -> None:
    ...
    comparison["evaluation_quality"]["state_model_note"] = (
        "The primary envelope replay asks whether the observed AS57807 tree can "
...
CONFIG = EventConfig(
    event_id='20140910_0030',
    event_name='Hijack event 20140910_0030',
    anomaly_type='hijack',
    canonical_prefix='177.200.2.0/23',
    root_asn=57807,
    victim_asn=52775,
