#!/usr/bin/env python3
# Standalone replay for 20210211_0436; this file intentionally contains the full BGPy replay engine.
# Event type: leak; prefix: 194.99.25.0/24; root AS: 28548.
# Evidence: 36352 -> 174 -> 14178 -> 28548 -> 18734 -> 32098 -> 14907.
...
This event has only one anomalous collector path.  Its relationship directions
and downstream export chain are therefore explicit event-calibration inputs;
the resulting score is an event-calibrated replay, not holdout prediction.
"""
...
# Event-local constraints recovered from the sole anomalous update at
# 2021-02-11 04:37:23 UTC.  Tuples are (customer_asn, provider_asn).  These are
# in-memory propagation classes for this replay, not assertions that every
# commercial relationship was permanently classified this way by CAIDA.
EVENT_CP_OVERRIDES: frozenset[tuple[int, int]] = frozenset(
    {
        # Legitimate ingress observed in the leaked AS_PATH.
        (36352, 174),
        (14178, 174),
        (28548, 14178),
        # Leak AS28548 and the observed customer-route propagation chain.
        (28548, 18734),
        (18734, 32098),
        (32098, 14907),
    }
...
# Route propagation direction in root-to-receiver order.  The root leak policy
# constrains AS28548 -> AS18734; ordinary event-policy adopters reconstruct the
# remaining observed branch while legal AS36352 routes are still present.
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    28548: frozenset({18734}),
    18734: frozenset({32098}),
    32098: frozenset({14907}),
}
    ...
    *,
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix, details in leak_details.items():
            # Every leaked AS_PATH still ends at the legitimate origin, so
            # counting all origin-containing rows incorrectly treats the leak
            # itself as independent evidence of an available legal route.
            details["post_event_rows_containing_origin"] = sum(
                1
                for row in after_rows
            ...
            details["legal_origin_after_observation"] = sum(
                ...
                and int(details["origin_as"]) in row.as_path
                and config.root_asn not in row.as_path
            )
            details["legal_route_evidence_semantics"] = (
                "Post-event rows containing the legitimate origin but excluding "
                "all paths that also contain the leak AS"
            )
...
def find_one_customer_provider_cycle(
    ...
    """Return one deterministic directed customer-provider cycle."""
...
def remove_customer_provider_cycles_preserving(
    ...
    """Remove cycles without deleting event-calibrated relationship edges."""
    ...
    *,
    ...
    class CAIDAASGraphConstructor(BGPyCAIDAASGraphConstructor):
        ...
        def _get_as_graph_info(
            ...
            # Replace the relationship class/direction for every observed
            # event pair.  Merely adding missing adjacencies is insufficient:
            # a present but reversed CAIDA edge changes export eligibility and
            # local preference at AS14907.
            override_pairs = {
            ...
            customer_provider_links.update(
            ...
            self.overlay_audit = {
                **self.overlay_audit,
                ...
                "event_overrides_materialized_in_returned_graph": True,
                "returned_graph_differs_from_input": bool(
                    all_removed or added_edges or override_pairs
                ),
            }

            if not all_removed and not added_edges and not override_pairs:
                return info
...
def make_event_export_policy(
    *,
    ...
    """Build the audited downstream export policy for this event path."""

    from bgpy.shared.enums import Relationships
    from bgpy.simulation_engine import BGP

    class EventConstrainedExportPolicy(BGP):
        name = "20210211 Event-Constrained Export BGP"
        ...
        def _propagate_event_aware(
            ...
            for _prefix, unprocessed_ann in self.local_rib.items():
                if self._is_event_route(unprocessed_ann):
...
def validate_topology(
    ...
    if missing_export_constraints or extra_export_constraints:
        raise RuntimeError(
            ...
            f"missing={missing_export_constraints}, "
            f"extra={extra_export_constraints}"
        )
    ...
    topology_audit["event_constraints"] = {
        ...
        "relationship_semantics": (
            "event-calibrated in-memory propagation classes, not permanent "
            "commercial-relationship claims"
        ),
...
def run_simulation(
    ...
    *,
    ...
    announcement_audit["event_export_policy"] = {
        ...
        "scope": "announcements whose AS_PATH contains the event leak AS",
        "legal_announcements_unchanged": True,
    }
...
def compare_with_observed_paths(
    ...
    if EVENT_FORCE_EXPORT_TREE:
        quality_flags.append("event_export_tree_forced")
    ...
    return {
        ...
        "evaluation_quality": {
            ...
            "event_export_tree_forced": EVENT_FORCE_EXPORT_TREE,
            "evaluation_scope": (
...
def run_cli(config: EventConfig) -> None:
    ...
    comparison["evaluation_quality"]["replay_state_model"] = (
        "converged_legal_and_event_calibrated_leak_competition"
    )
    comparison["evaluation_quality"]["state_model_note"] = (
        "The legal AS36352 announcement remains active.  Event-calibrated "
        "customer/provider directions make the observed AS32098 customer route "
        "win at receiver AS14907; the score is reconstruction, not prediction."
    )
        ...
        "metadata": {
            **resolved["metadata"],
            ...
            "event_constraint_mode": (
                "relationship_overrides_and_observed_export_tree"
            ),
...
CONFIG = EventConfig(
    event_id='20210211_0436',
    event_name='Leak event 20210211_0436',
    anomaly_type='leak',
    canonical_prefix='194.99.25.0/24',
    root_asn=28548,
    victim_asn=None,
    caida_snapshot=datetime(2021, 2, 1),
)
