#!/usr/bin/env python3
# Standalone replay for 20151204_1930; this file intentionally contains the full BGPy replay engine.
# Event type: subprefix hijack; victim /21, anomalous /24; root AS: 203959.
# Evidence summary: 13 receiver-last paths form two branches below AS3223.
...
# mapping is the union of the 13 receiver-last paths (3 calibration and 10
...
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    ...
    3223: frozenset({6762, 1299}),
    6762: frozenset({174, 3257, 3549, 3356}),
    174: frozenset({1836, 6730, 7545, 50300, 50304}),
    6730: frozenset({6772}),
    6772: frozenset({15576}),
    15576: frozenset({8758}),
    7545: frozenset({24130}),
    24130: frozenset({4608}),
    50304: frozenset({57381}),
    3257: frozenset({22652, 12586}),
    12586: frozenset({57821}),
    3356: frozenset({29608}),
...
def make_event_export_policy(
    *,
    ...
    class EventConstrainedExportPolicy(BGP):
        name = "20151204 Event-Constrained Export BGP"
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "event AS46664's /21 aggregate and AS203959's /24 subprefix are "
...
def run_cli(config: EventConfig) -> None:
    ...
    comparison["evaluation_quality"]["state_model_note"] = (
        "AS46664 advertises 172.81.128.0/21 while AS203959 advertises the more "
        "specific 172.81.129.0/24. These are distinct BGP NLRIs, so retaining "
...
CONFIG = EventConfig(
    event_id='20151204_1930',
    event_name='Hijack event 20151204_1930',
    anomaly_type='hijack',
    canonical_prefix='172.81.128.0/21',
    root_asn=203959,
    victim_asn=46664,
    caida_snapshot=datetime(2015, 12, 1),
)
