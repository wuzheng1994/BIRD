#!/usr/bin/env python3
# Standalone replay for 20170825_0322; this file intentionally contains the full BGPy replay engine.
# Event type: peer-to-provider route leak; two /21 prefixes; leak AS: 15169.
# Evidence: 57332 -> 21395 -> 15169 -> 701 -> receivers in event AS_PATH order.
...
# These constraints represent the historical AS15169 leak envelope.
...
EVENT_CP_OVERRIDES = frozenset(
    {
        # Ingress: AS21395 can export AS57332's customer route to peer AS15169.
        (57332, 21395),
        # Violation: AS15169 exports a peer-learned route to provider AS701.
        (15169, 701),
        # Downstream customer branches after AS701.
        (1136, 286),
        (3333, 1136),
        (29608, 5511),
        (8220, 5511),
...
# Event-observed propagation tree.  For a route whose AS_PATH contains leak AS
# 15169, each key AS may export only to the listed next-hop ASNs. Both /21
# prefixes share the same four receiver branches. Legal paths from AS57332 that
# do not traverse AS15169 retain ordinary BGP export behavior.
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    15169: frozenset({701}),
    701: frozenset({286, 5511, 2497}),
    286: frozenset({1136}),
    1136: frozenset({3333}),
    5511: frozenset({29608, 8220}),
    8220: frozenset({15576}),
...
def make_event_export_policy(
    *,
    ...
    class EventConstrainedExportPolicy(BGP):
        name = "20170825 Event-Constrained Export BGP"
        ...
        def _policy_propagate(
            ...
            # Normal/legal announcements must retain ordinary BGP export. For
            # a leak the origin remains AS57332, so identify the abnormal route
            # by the presence of AS15169 in its AS_PATH rather than ann.origin.
            if (
                self.event_root_asn not in ann.as_path
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "primary experiment reconstructs the transient AS15169 leak envelope. "
...
def run_cli(config: EventConfig) -> None:
    ...
    comparison["evaluation_quality"]["state_model_note"] = (
        ...
        "allowing the simultaneously available legal AS57332 route to suppress "
...
CONFIG = EventConfig(
    event_id='20170825_0322',
    event_name='Leak event 20170825_0322',
    anomaly_type='leak',
    canonical_prefix='5.206.240.0/20',
    root_asn=15169,
    victim_asn=None,
    caida_snapshot=datetime(2017, 8, 1),
)
