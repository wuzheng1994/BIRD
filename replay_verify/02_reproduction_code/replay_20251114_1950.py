#!/usr/bin/env python3
# Standalone replay for 20251114_1950; this file intentionally contains the full BGPy replay engine.
# Event type: leak; prefix: 102.135.220.0/24; root AS: 30998.
# Evidence summary: No narrative report; derive anchors from the event RIB.
...
def resolve_event_inputs(
    ...
    *,
    ...
    history_path = event_dir / "history_rib.csv"
    before_path = event_dir / "rib_before_incident.csv"
    after_path = event_dir / "rib_after_incident.csv"
    history_rows = load_rib_rows(history_path) if history_path.exists() else []
    before_rows = load_rib_rows(before_path)
    after_rows = load_rib_rows(after_path)

    selected_rows, primary_observation_audit = select_observation_rows(
        config, after_rows, observation_mode
    )
    observation_source = "rib_after_incident"
    observation_fallback_reason: str | None = None
    observation_audit = {
        **primary_observation_audit,
        "source": observation_source,
        "primary_source": "rib_after_incident",
        "fallback_used": False,
    }

    # This route-leak data slice ends with a complete withdrawal: the after
    # file contains only its CSV header, while the stable history snapshot and
    # the before file contain the AS30998 leak paths.  A post-event comparison
    # is therefore undefined.  Replay the last observable pre-withdrawal leak
    # envelope instead, preferring history_rib.csv because it is the compact
    # stable snapshot rather than a collection of path-exploration updates.
    if config.anomaly_type == "leak" and not selected_rows:
        observation_fallback_reason = (
            "rib_after_incident_has_no_rows"
            if not after_rows
            else "rib_after_incident_has_no_configured_leak_as_path"
        )
        fallback_candidates = (
            ("history_rib_pre_withdrawal", history_rows),
            ("rib_before_incident_pre_withdrawal", before_rows),
        )
        for candidate_source, candidate_rows in fallback_candidates:
            fallback_rows, fallback_audit = select_observation_rows(
                config, candidate_rows, observation_mode
            )
            if fallback_rows:
                selected_rows = fallback_rows
                observation_source = candidate_source
                observation_audit = {
                    **fallback_audit,
                    "source": observation_source,
                    "primary_source": "rib_after_incident",
                    "fallback_used": True,
                    "fallback_reason": observation_fallback_reason,
                    "primary_source_audit": primary_observation_audit,
                    "state_semantics": "pre_withdrawal_leak_envelope",
                    "note": (
                        "rib_after_incident.csv contains no comparable leak "
                        "announcement. The observed event state is the stable "
                        "pre-withdrawal leak envelope from history_rib.csv; "
                        "calibration and evaluation receivers remain disjoint."
                    ),
                }
                break
    ...
    calibration_paths, calibration_peer_path_mismatches = serialize_observed_paths(
        config, calibration_rows
    )

    evaluation_pair_keys = {
        (row.prefix, row.peer_asn) for row in evaluation_rows
    }
    observation_uses_pre_withdrawal_data = observation_source in {
        "history_rib_pre_withdrawal",
        "rib_before_incident_pre_withdrawal",
    }
    pre_topology_rows = (
        [
            row
            for row in before_rows
            if config.root_asn not in row.as_path
            and (row.prefix, row.peer_asn) not in evaluation_pair_keys
        ]
        if observation_uses_pre_withdrawal_data
        else list(before_rows)
    )
    pre_edges = rib_path_edges(pre_topology_rows, config.canonical_prefix)
    calibration_edges = rib_path_edges(calibration_rows, config.canonical_prefix)

    resolved: dict[str, Any] = {
        "metadata": asdict(config),
        "event_dir": str(event_dir.resolve()),
        "history_rib_file": str(history_path.resolve()),
        ...
        "origin_audit": {
            ...
            "history_unique_path_origins": origin_count_audit(
                history_rows, config.canonical_prefix
            ),
        ...
        "topology_overlay_candidates": {
            ...
            "pre_rib_paths": rib_propagation_paths(
                pre_topology_rows, config.canonical_prefix
            ),
            ...
            "pre_topology_source": "rib_before_incident_filtered",
            "pre_topology_row_count": len(pre_topology_rows),
            "pre_topology_excluded_root_path_count": (
                sum(config.root_asn in row.as_path for row in before_rows)
                if observation_uses_pre_withdrawal_data
                else 0
            ),
            "evaluation_receiver_rows_excluded_from_pre_topology": (
                observation_uses_pre_withdrawal_data
            ),
            "pre_topology_root_path_count": sum(
                config.root_asn in row.as_path for row in pre_topology_rows
            ),
            "pre_topology_evaluation_pair_overlap_count": sum(
                (row.prefix, row.peer_asn) in evaluation_pair_keys
                for row in pre_topology_rows
            ),
        },
        "input_audit": {
            ...
            "calibration_peer_path_mismatch_count": (
                calibration_peer_path_mismatches
            ),
            "real_path_comparison_available": bool(observed_paths),
            "observation_status": (
                (
                    "anomalous_paths_found_in_pre_withdrawal_fallback"
                    if observation_uses_pre_withdrawal_data
                    else "anomalous_paths_found"
                )
                if observed_paths
                else "no_root_as_path_in_available_rib_sources"
            ),
            "observation_note": (
                (
                    "The after RIB is empty; comparison uses a receiver-holdout "
                    "pre-withdrawal leak envelope from history_rib.csv."
                    if observed_paths and observation_uses_pre_withdrawal_data
                    else None
                )
                if observed_paths else (
                    f"No AS{config.root_asn} anomalous path was found in "
                    "the after, history, or before RIB sources. The configured "
                    "event can still be simulated, but similarity is unavailable."
                )
            ),
        },
    }
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix, details in leak_details.items():
            legal_origin_observation_count = sum(
                1
                for row in selected_rows
                if row.prefix == prefix and int(details["origin_as"]) in row.as_path
            )
            # Keep the established key for compatibility with the common
            # announcement builder, and add the semantically precise name for
            # this pre-withdrawal fallback event.
            details["legal_origin_after_observation"] = (
                legal_origin_observation_count
            )
            details["legal_origin_during_observation"] = (
                legal_origin_observation_count
            )
            details["legal_origin_observation_source"] = observation_source
...
def compare_with_observed_paths(
    ...
    observation_source = resolved.get("input_audit", {}).get(
        "observation_source", "rib_after_incident"
    )
    pre_withdrawal_fallback_used = observation_source in {
        "history_rib_pre_withdrawal",
        "rib_before_incident_pre_withdrawal",
    }
    ...
    if pre_withdrawal_fallback_used:
        quality_flags.append("pre_withdrawal_rib_used_because_after_rib_empty")
    ...
    return {
        ...
        "unavailable_reason": (
            ...
            if observed_paths
                ...
                "No anomalous path containing the configured root AS was found "
                "in the after, history, or before RIB sources; similarity "
                "metrics are therefore null rather than zero."
            )
        ),
        "comparison": (
            "bgpy_structural_replay_vs_pre_withdrawal_history_rib"
            if pre_withdrawal_fallback_used
            else "bgpy_structural_replay_vs_real_rib_after"
        ),
        "path_direction": "root_as_to_receiver",
        "note": (
            (
                "The after RIB is empty, so the receiver-holdout comparison "
                "uses the stable pre-withdrawal leak envelope. It evaluates "
                "route-leak structure, not the post-withdrawal empty state."
            )
            if pre_withdrawal_fallback_used
            else (
                "Observed paths are a sparse collector sample. Coverage metrics "
                "are valid, but simulation precision cannot be inferred from "
                "unobserved paths."
            )
        ),
        "evaluation_quality": {
            ...
            "observation_source": observation_source,
            "pre_withdrawal_fallback_used": pre_withdrawal_fallback_used,
            "evaluation_scope": (
                "receiver_holdout_pre_withdrawal_leak_envelope"
                if pre_withdrawal_fallback_used
                else "receiver_holdout_or_all_post_event_observations"
            ),
            "evaluation_receiver_rows_excluded_from_pre_topology": resolved.get(
                "topology_overlay_candidates", {}
            ).get("evaluation_receiver_rows_excluded_from_pre_topology", False),
            "validation_split": split_audit,
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "the selected event observation state, or never. Default: "
            "after-observed."
        ),
    )
    parser.add_argument(
...
def run_cli(config: EventConfig) -> None:
    ...
    print(
        "[+] Observation source: "
        f"{resolved['input_audit'].get('observation_source', 'unknown')}"
    )
    if resolved.get("observation_selection", {}).get("fallback_used", False):
        print(
            "[!] rib_after_incident.csv is empty; evaluating the disjoint "
            "pre-withdrawal leak envelope instead of a post-withdrawal path state"
        )
    ...
    complete_result = {
        "metadata": {
            **resolved["metadata"],
            ...
            "legal_route_mode": args.legal_route_mode,
            "path_source": args.path_source,
            "observation_source": resolved["input_audit"].get(
                "observation_source"
            ),
...
CONFIG = EventConfig(
    event_id='20251114_1950',
    event_name='Leak event 20251114_1950',
    anomaly_type='leak',
    canonical_prefix='102.135.220.0/24',
    root_asn=30998,
    victim_asn=None,
