#!/usr/bin/env python3
# Standalone replay for 20220817_fp; this file intentionally contains the full BGPy replay engine.
# Event type: Type-1 S|1 forged-origin hijack (FP candidate); prefix: 44.235.216.0/24.
# Attacker A=AS209243 seeds as_path=(A, V) with forged origin V=AS14618.
# Pre-event legal origin for the realistic counterfactual: AS16509 on covering 44.224.0.0/11.
...
LegalRouteMode = Literal["always", "after-observed", "withdrawn", "prefix-aware"]
...
# origin = Type-0 attacker-origin hijack; forged-origin = Type-1 ... A V
HIJACK_FORM = "forged-origin"
FORGED_ORIGIN_ASN = 14618


def is_forged_origin_hijack() -> bool:
    return HIJACK_FORM == "forged-origin"


def forged_origin_as() -> int:
    return int(FORGED_ORIGIN_ASN)
...
def covers_canonical(prefix: str, canonical_prefix: str) -> bool:
    candidate = ip_network(prefix)
    canonical = ip_network(canonical_prefix)
    if candidate.prefixlen == 0:
        return False
    return candidate.version == canonical.version and canonical.subnet_of(candidate)
...
def normalize_observed_path(
    ...
    *,
    ...
    if config.anomaly_type in {"hijack", "recovery"}:
        if config.anomaly_type == "hijack" and is_forged_origin_hijack():
            forged = forged_origin_as()
            if len(row.as_path) < 2:
                return None
            if row.as_path[-1] != forged or row.as_path[-2] != config.root_asn:
                return None
...
def prefix_in_event_scope(
    prefix: str,
    canonical_prefix: str,
    *,
    include_covering: bool = False,
) -> bool:
    if is_within_canonical(prefix, canonical_prefix):
        return True
    return include_covering and covers_canonical(prefix, canonical_prefix)
...
def legal_origin_after_path_count(
    after_rows: Sequence[RibRow],
    victim_asn: int,
    victim_prefix: str,
) -> int:
    """Count post-event paths still originated by the pre-event legal origin.

    More-specific announcements of the covering victim prefix count, so S|1
    events are not treated as 'legal origin gone' just because after RIB only
    contains the hijacked more-specific.
    """

    victim_net = ip_network(victim_prefix)
    count = 0
    for row in after_rows:
        if not row.as_path or int(row.as_path[-1]) != int(victim_asn):
            continue
        candidate = ip_network(row.prefix)
        if candidate.version != victim_net.version:
            continue
        if candidate == victim_net or candidate.subnet_of(victim_net):
            count += 1
    return count


def attacker_first_hop_overlay(
    config: EventConfig,
    observed_paths: Sequence[dict[str, Any]],
) -> dict[str, Any]:
    """Observed A→next-hop edges, excluding the forged A–V adjacency."""

    forged = forged_origin_as() if is_forged_origin_hijack() else None
    hops: set[int] = set()
    skipped_forged: list[int] = []
    for item in observed_paths:
        path = [int(asn) for asn in item.get("propagation_path", [])]
        if len(path) < 2 or path[0] != int(config.root_asn):
            continue
        hop = int(path[1])
        if forged is not None and hop == int(forged):
            skipped_forged.append(hop)
            continue
        hops.add(hop)
    return {
        "attacker_as": int(config.root_asn),
        "forged_origin_as": forged,
        "first_hops": sorted(hops),
        "skipped_forged_origin_first_hops": sorted(set(skipped_forged)),
        "edges": [
            [int(config.root_asn), hop] for hop in sorted(hops)
        ],
        "cp_links": [
            {
                "customer_as": int(config.root_asn),
                "provider_as": hop,
                "note": (
                    "Attacker is attached as a customer of the observed first "
                    "hop so Gao-Rexford can re-advertise toward providers."
                ),
            }
...
def origin_count_audit(
    ...
    *,
    ...
    unique_paths = {
        ...
        or (include_covering and covers_canonical(row.prefix, canonical_prefix))
    }
    ...
    *,
    ...
    resolved: dict[str, Any] = {
        ...
        "topology_overlay_candidates": {
            ...
            "pre_rib_includes_covering_prefixes": True,
            "attacker_first_hop_overlay": first_hop_overlay,
    ...
    if config.anomaly_type in {"hijack", "recovery"}:
        ...
        if config.anomaly_type == "hijack" and is_forged_origin_hijack():
            ...
            after_forged = sum(
                ...
                for row in after_rows
                if is_within_canonical(row.prefix, config.canonical_prefix)
                and len(row.as_path) >= 2
                and row.as_path[-1] == forged
                and row.as_path[-2] == config.root_asn
            )
            orientation_supported = before_other > 0 and after_forged > 0
            resolved["configuration_orientation_audit"] = {
                ...
                "after_forged_adjacency_path_count": after_forged,
                "supported_by_origin_transition": orientation_supported,
                "note": (
                    "Type-1: the pre-event legal origin must be visible before "
                    "the event and the forged A-V adjacency must appear after."
                ),
            }
        else:
    ...
    if config.anomaly_type == "hijack":
        ...
        resolved["hijack"] = {
            ...
            "hijack_form": HIJACK_FORM,
            "forged_origin_as": (
                forged_origin_as() if is_forged_origin_hijack() else None
            ),
            ...
            "legal_origin_after_observation": {
                anomalous_prefix: {
                    ...
                    "covering_prefix_fallback": (
                        victim_prefixes[anomalous_prefix] != anomalous_prefix
                    ),
                    "after_path_count": legal_origin_after_path_count(
                        after_rows,
                        config.victim_asn,
                        victim_prefixes[anomalous_prefix],
                    ),
                }
                for anomalous_prefix in prefixes
            },
...
def build_as_graph(
    ...
    *,
    ...
    first_hop_overlay = overlay_candidates.get("attacker_first_hop_overlay", {})
    first_hop_edges = {
        tuple(sorted((int(edge[0]), int(edge[1]))))
        for edge in first_hop_overlay.get("edges", [])
    }
    first_hop_cp = {
        tuple(sorted((int(link["customer_as"]), int(link["provider_as"])))): (
            int(link["customer_as"]),
            int(link["provider_as"]),
        )
        for link in first_hop_overlay.get("cp_links", [])
    }
    if topology_overlay_mode == "none":
        ...
        requested_overlay_edges = pre_rib_edges | first_hop_edges
        requested_overlay_paths = pre_rib_paths
    else:
        requested_overlay_edges = pre_rib_edges | calibration_edges | first_hop_edges
    ...
    class CAIDAASGraphConstructor(BGPyCAIDAASGraphConstructor):
        ...
        def _get_as_graph_info(
            ...
            for left, right in added_edges:
                ...
                forced_cp = first_hop_cp.get((left, right))
                if forced_cp is not None:
                    selected = ("cp", forced_cp[0], forced_cp[1])
                else:
            ...
            self.overlay_audit = {
                **self.overlay_audit,
                ...
                "attacker_first_hop_overlay_applied": bool(first_hop_cp),
                "attacker_first_hops": list(
                    first_hop_overlay.get("first_hops", [])
                ),
            }
...
def build_announcements(
    ...
    *,
    ...
    def should_seed_legal(
        after_path_count: int,
        *,
        covering_baseline: bool = False,
    ) -> bool:
        if legal_route_mode == "always":
            return True
        if legal_route_mode == "withdrawn":
            return False
        if legal_route_mode == "prefix-aware":
            if covering_baseline:
                return True
            # Same-prefix legal origin of a different ASN would compete with
            # Type-1 (A, V) and can erase the observed hijack envelope.
            if is_forged_origin_hijack() and forged_origin_as() != int(
                config.victim_asn
            ):
                return False
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix in victim_prefixes:
            ...
            covering_baseline = any(
                bool(item.get("covering_prefix_fallback"))
                for item in availability.values()
                if item["victim_prefix"] == prefix
            )
            if not should_seed_legal(
                after_path_count,
                covering_baseline=covering_baseline,
            ):
        ...
        for prefix in resolved["anomalous_prefixes"]:
            if is_forged_origin_hijack():
                forged = forged_origin_as()
                announcement = Announcement(
                    prefix=prefix,
                    as_path=(config.root_asn, forged),
                ...
                audit["announcements"].append(
                    {
                        ...
                        "type": "type1_forged_origin_hijack",
                        "as_path_view": [config.root_asn, forged],
                        "forged_origin_as": forged,
                    }
                )
            else:
...
def validate_topology(
    ...
    if config.anomaly_type == "hijack" and is_forged_origin_hijack():
        forged = forged_origin_as()
        root_obj = as_graph.as_dict[config.root_asn]
        adjacency = relationship_name(root_obj, forged)
        topology_audit["type1_forged_adjacency"] = {
            "attacker_as": config.root_asn,
            "forged_origin_as": forged,
            "caida_relationship": adjacency,
            "present_in_caida": adjacency != "not-neighbor",
            "note": (
                "Type-1 treats a missing CAIDA adjacency as the forged edge; "
                "an existing adjacency is a legitimate-upstream alternative."
            ),
        }
            ...
            for ann in candidate_anns:
                ...
                if config.anomaly_type in {"hijack", "recovery"}:
                    if (
                        ...
                        and is_forged_origin_hijack()
                    ):
                        forged = forged_origin_as()
                        announcement_origin = int(
                            getattr(ann, "origin", ann.as_path[-1])
                        )
                        is_abnormal = (
                            announcement_origin == forged
                            and len(ann.as_path) >= 2
                            and int(ann.as_path[-2]) == config.root_asn
                        )
                    else:
...
def compare_with_observed_paths(
    ...
    first_hop_overlay = (
        resolved.get("topology_overlay_candidates", {})
        .get("attacker_first_hop_overlay", {})
    )
    if (
        simulation.get("topology_overlay_mode") != "none"
        and first_hop_overlay.get("first_hops")
    ):
        quality_flags.append("reproduction_attacker_first_hop_overlay")
    covering_pre_rib = resolved.get("topology_overlay_candidates", {}).get(
        "pre_rib_includes_covering_prefixes"
    )
    if covering_pre_rib and simulation.get("topology_overlay_mode") != "none":
        quality_flags.append("covering_prefix_pre_rib_overlay")
    if simulation.get("legal_route_mode") == "prefix-aware":
        quality_flags.append("prefix_aware_legal_origin_seeding")
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--legal-route-mode",
        choices=("always", "after-observed", "withdrawn", "prefix-aware"),
        default="after-observed",
        help=(
            "Legal origin seeding. prefix-aware always seeds a covering-prefix "
            "baseline and uses after-observed for the same prefix. "
            "Default: after-observed."
        ),
    )
    parser.add_argument(
...
CONFIG = EventConfig(
    event_id='20220817_fp',
    event_name='Type-1 FP more-specific event 20220817_fp',
    anomaly_type='hijack',
    canonical_prefix='44.235.216.0/24',
    root_asn=209243,
    victim_asn=16509,
    caida_snapshot=datetime(2024, 10, 1),
)
