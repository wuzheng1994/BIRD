#!/usr/bin/env python3
# Standalone replay for 20171106_1747; this file intentionally contains the full BGPy replay engine.
# Event type: provider-to-peer route leak; prefix /18; leak AS: 1239.
# Evidence: 33588 -> 3356 -> 1239 -> {1257, 4637} -> receivers.
...
# These constraints represent the historical AS1239 leak envelope.
...
EVENT_CP_OVERRIDES = frozenset(
    {
        # AS3356 learns the legal route from customer/origin AS33588.
        (33588, 3356),
        # Ingress: leak AS1239 learns the route from provider AS3356.
        (1239, 3356),
        # Downstream customer branches after the two leak targets.
        (3333, 1257),
        (1221, 4637),
        (4608, 1221),
    }
...
EVENT_PEER_OVERRIDES: frozenset[tuple[int, int]] = frozenset(
    {
        # Violation: provider-learned route exported to peers.
        (1239, 1257),
        (1239, 4637),
    }
)
...
# 1239, each key AS may export only to the listed next-hop ASNs. Legal paths
# from AS33588 that do not traverse AS1239 retain ordinary BGP export behavior.
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    1239: frozenset({1257, 4637}),
    1257: frozenset({3333}),
    4637: frozenset({1221}),
    1221: frozenset({4608}),
}
...
def make_event_export_policy(
    *,
    ...
    class EventConstrainedExportPolicy(BGP):
        name = "20171106 Event-Constrained Export BGP"
        ...
        def _policy_propagate(
            ...
            # a leak the origin remains AS33588, so identify the abnormal route
            # by the presence of AS1239 in its AS_PATH rather than ann.origin.
            if (
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "primary experiment reconstructs the transient AS1239 leak envelope. "
...
def run_cli(config: EventConfig) -> None:
    ...
    comparison["evaluation_quality"]["state_model_note"] = (
        ...
        "allowing the simultaneously available legal AS33588 route to suppress "
...
CONFIG = EventConfig(
    event_id='20171106_1747',
    event_name='Leak event 20171106_1747',
    anomaly_type='leak',
    canonical_prefix='47.37.0.0/18',
    root_asn=1239,
    victim_asn=None,
    caida_snapshot=datetime(2017, 11, 1),
)
