#!/usr/bin/env python3
# Standalone replay for 20190624_1030; this file intentionally contains the full BGPy replay engine.
# Event type: multi-stage route leak; prefix /23; primary leak AS: 396531.
# Evidence: 393436 -> 3356 -> 33154 -> 396531 -> 701 -> 1273 -> 3333.

"""Replay one historical BGP anomaly and compare it with observed paths.

The module supports three experiment designs:

* Prefix hijack: inject the configured attacker route and optionally retain the
  observed legitimate origin according to ``--legal-route-mode``.
...
# These constraints represent the historical AS396531 leak envelope.
# Relationship overrides are calibrated propagation classes,
# not claims about permanent commercial contracts between every AS pair.
EVENT_ATTACK_FIRST_HOPS: tuple[int, ...] = ()
EVENT_ANCHOR_RECV_RELATIONSHIP = "peer"
...
EVENT_CP_OVERRIDES = frozenset(
    {
        (393436, 3356),
        (33154, 3356),
        # Primary leak AS396531 is a customer of both adjacent providers.
        (396531, 33154),
        (396531, 701),
        # AS1273 exports the peer-learned route toward provider AS3333.
        (1273, 3333),
    }
)
...
# Event-observed propagation tree. For a route whose AS_PATH contains primary
# leak AS396531, each key AS may export only to the listed next-hop ASNs. The
# AS1273 -> AS3333 edge is a secondary export violation and is forced explicitly.
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    396531: frozenset({701}),
    701: frozenset({1273}),
    1273: frozenset({3333}),
}
EVENT_FORCE_EXPORT_TREE = True
...
def find_one_customer_provider_cycle(
    relationships: Iterable[tuple[int, int]],
) -> tuple[tuple[int, int], ...]:
    """Return one deterministic directed cycle, or an empty tuple.

    The iterative DFS also reconstructs the tree path when its back edge is
    protected by an event override.  That lets the caller remove a different,
    non-event edge from the same cycle instead of discarding historical event
    evidence merely because the protected edge happened to be visited last.
    """
    ...
    for start_asn in sorted(nodes):
        ...
        while stack:
            ...
            if color[provider_asn] == 0:
                parent[provider_asn] = current_asn
                color[provider_asn] = 1
                stack.append(
                    (provider_asn, iter(sorted(adjacency[provider_asn])))
                )
            elif color[provider_asn] == 1:
                # provider_asn -> ... -> current_asn is the DFS-tree portion;
                # current_asn -> provider_asn closes the directed cycle.
                reverse_nodes = [current_asn]
                while reverse_nodes[-1] != provider_asn:
                    reverse_nodes.append(parent[reverse_nodes[-1]])
                cycle_nodes = list(reversed(reverse_nodes))
                cycle_edges = [
                    (left, right)
                    for left, right in zip(cycle_nodes, cycle_nodes[1:])
                ]
                cycle_edges.append((current_asn, provider_asn))
                return tuple(cycle_edges)
    return ()


def remove_customer_provider_cycles_preserving(
    relationships: Iterable[tuple[int, int]],
    protected_edges: Iterable[tuple[int, int]] = (),
) -> tuple[set[tuple[int, int]], set[tuple[int, int]]]:
    """Make the CP graph acyclic without deleting protected event edges.

    Ordinary DFS back edges are removed first so this remains compatible with
    the script's original deterministic cycle handling.  If the DFS back edge
    is protected, one non-protected edge on that exact cycle is removed.  A
    RuntimeError is reserved for a cycle made entirely of protected edges.
    """

    remaining = {
        (int(customer), int(provider)) for customer, provider in relationships
    }
    protected = {
        (int(customer), int(provider)) for customer, provider in protected_edges
    }
    removed: set[tuple[int, int]] = set()

    while True:
        back_edges = find_customer_provider_back_edges(remaining)
        if not back_edges:
            return remaining, removed

        removable_back_edges = back_edges.difference(protected)
        if removable_back_edges:
            remaining.difference_update(removable_back_edges)
            removed.update(removable_back_edges)
            continue

        cycle_edges = find_one_customer_provider_cycle(remaining)
        if not cycle_edges:
            raise RuntimeError(
                "Customer-provider cycle detection became inconsistent"
            )
        removable_cycle_edges = sorted(set(cycle_edges).difference(protected))
        if not removable_cycle_edges:
            raise RuntimeError(
                "Event relationship overrides themselves form a protected "
                f"customer-provider cycle: {list(cycle_edges)}"
            )
        edge_to_remove = removable_cycle_edges[0]
        remaining.remove(edge_to_remove)
    ...
    *,
    ...
    class CAIDAASGraphConstructor(BGPyCAIDAASGraphConstructor):
        ...
        cycle_audit: dict[str, Any] = {
            "method": "deterministic_dfs_preserving_event_cp_edges",
            "removed_customer_provider_edge_count": 0,
            "removed_customer_provider_edges": [],
            "removed_before_event_override": [],
            "removed_after_event_override": [],
            "protected_event_cp_edges": [],
        ...
        def _get_as_graph_info(
            ...
            _, removed_before_override = (
                remove_customer_provider_cycles_preserving(
                    relationships,
                    protected_edges=EVENT_CP_OVERRIDES,
                )
            )
            self.cycle_audit = {
                "method": "deterministic_dfs_preserving_event_cp_edges",
                "removed_customer_provider_edge_count": len(
                    removed_before_override
                ),
                "removed_customer_provider_edges": [
                    {"customer_as": customer, "provider_as": provider}
                    for customer, provider in sorted(removed_before_override)
                ],
                "removed_before_event_override": [
                    {"customer_as": customer, "provider_as": provider}
                    for customer, provider in sorted(removed_before_override)
            ...
            customer_provider_links = frozenset(
                link
                for link in info.customer_provider_links
                if (link.customer_asn, link.provider_asn)
                not in removed_before_override
            )
            ...
            post_override_relationships, removed_after_override = (
                remove_customer_provider_cycles_preserving(
                    {
                        (link.customer_asn, link.provider_asn)
            ...
            customer_provider_links = frozenset(
                CustomerProviderLink(customer_asn=customer, provider_asn=provider)
                for customer, provider in post_override_relationships
            )
            all_removed = removed_before_override | removed_after_override
            self.cycle_audit = {
                **self.cycle_audit,
                "removed_customer_provider_edge_count": len(all_removed),
                "removed_customer_provider_edges": [
                    {"customer_as": customer, "provider_as": provider}
                    for customer, provider in sorted(all_removed)
                ],
                "removed_after_event_override": [
                    {"customer_as": customer, "provider_as": provider}
                    for customer, provider in sorted(removed_after_override)
                ],
            }
            self.overlay_audit = {
                **self.overlay_audit,
            ...
            self.overlay_audit = {
                **self.overlay_audit,
                ...
                "returned_graph_differs_from_input": bool(
                    all_removed or added_edges or event_overrides_requested
                ),
            }
            if (
                not all_removed
                and not added_edges
                and not event_overrides_requested
            ):
                return info
    ...
    *,
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix in resolved["anomalous_prefixes"]:
            ...
            for first_hop_asn in EVENT_ATTACK_FIRST_HOPS:
                ...
                anchored = Announcement(
                    ...
                    # first hop selected the configured attacker route.
                ...
                audit["announcements"].append(
                    {
                        ...
                        "calibration_note": (
                            ...
                            f"the AS{config.root_asn} route; not an independent "
                            "prediction."
                        ),
                    }
                )
        ...
        for prefix, details in sorted(resolved["route_leak"].items()):
            ...
            peer_pair = frozenset({config.root_asn, origin_neighbor})
            event_peer_pairs = {
                frozenset({left, right}) for left, right in EVENT_PEER_OVERRIDES
            }
            if peer_pair in event_peer_pairs:
                relationship_source = "event_peer_override"
            elif (
                (config.root_asn, origin_neighbor) in EVENT_CP_OVERRIDES
                or (origin_neighbor, config.root_asn) in EVENT_CP_OVERRIDES
            ):
                relationship_source = "event_customer_provider_override"
            else:
...
def make_event_export_policy(
    *,
    ...
    """Build a BGP policy that follows a multi-stage observed export tree.

    Normal announcements retain Gao--Rexford export eligibility. An anomalous
    route containing the configured primary leak AS is sent only over the
    observed event edge, even when that edge is itself a secondary export
    violation. This is required for AS1273 -> AS3333 in this event.
    """
    ...
    class EventConstrainedExportPolicy(BGP):
        name = "20190624 Multi-Stage Event Export BGP"
        ...
        def _is_event_route(self, ann: Any) -> bool:
            return (
                ann.prefix in self.event_prefixes
                and self.event_root_asn in ann.as_path
            )

        def _propagate_event_aware(
            self,
            propagate_to: Any,
            send_rels: set[Any],
        ) -> None:
            ...
            for _prefix, unprocessed_ann in self.local_rib.items():
                is_event_route = self._is_event_route(unprocessed_ann)
                if is_event_route:
                    allowed_targets = self.event_export_targets.get(
                        int(self.as_.asn), frozenset()
                    )
                    eligible_neighbors = tuple(
                        neighbor
                        for neighbor in neighbors
                        if int(neighbor.asn) in allowed_targets
                    )
                elif unprocessed_ann.recv_relationship in send_rels:
                    eligible_neighbors = tuple(neighbors)
                else:
                    continue
                ...
                for neighbor in eligible_neighbors:
                    if not self._prev_sent(neighbor, ann):
                        self._process_outgoing_ann(
                            neighbor, ann, propagate_to, send_rels
                        )

        def propagate_to_providers(self) -> None:
            self._propagate_event_aware(
                Relationships.PROVIDERS,
...
def validate_topology(
    ...
    topology_audit["event_constraints"] = {
        ...
        "export_filter_scope": (
            "only anomalous prefixes whose AS_PATH contains the configured leak AS"
        ),
        "event_export_tree_forces_observed_edges": EVENT_FORCE_EXPORT_TREE,
        "secondary_export_violation": {
            "leak_as": 1273,
            "learned_from_as": 701,
            "exported_to_as": 3333,
        },
...
def compare_with_observed_paths(
    ...
    if EVENT_FORCE_EXPORT_TREE:
        quality_flags.append("multi_stage_export_violation_forced")
    observation_mode = resolved.get("observation_selection", {}).get("mode")
    if observation_mode == "all-updates":
        quality_flags.append("transient_updates_mixed_in_static_comparison")
    elif observation_mode == "receiver-last":
        quality_flags.append("receiver_last_envelope_not_coherent_global_state")
    ...
    return {
        ...
        "evaluation_quality": {
            ...
            "multi_stage_export_violation_forced": EVENT_FORCE_EXPORT_TREE,
            "evaluation_scope": (
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "rib_after_incident.csv, or never. Default: withdrawn, because the "
            "primary experiment reconstructs the transient AS396531 leak envelope. "
            "Use after-observed for a separate converged legal-versus-leaked-path "
...
def run_cli(config: EventConfig) -> None:
    ...
    replay_state_model = (
        "event_route_leak_propagation_envelope"
        if args.legal_route_mode == "withdrawn"
        else "converged_legal_and_leaked_path_competition"
    )
    comparison["evaluation_quality"]["replay_state_model"] = replay_state_model
    comparison["evaluation_quality"]["state_model_note"] = (
        "The primary run reconstructs the receiver-last leak envelope without "
        "allowing the simultaneously available legal AS393436 route to suppress "
        "the transient leaked path. Use --legal-route-mode after-observed as a "
        "separate converged competition experiment."
    )
...
CONFIG = EventConfig(
    event_id='20190624_1030',
    event_name='Leak event 20190624_1030',
    anomaly_type='leak',
    canonical_prefix='97.107.66.0/23',
    root_asn=396531,
    victim_asn=None,
    caida_snapshot=datetime(2019, 6, 1),
)
