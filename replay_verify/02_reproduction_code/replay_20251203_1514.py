#!/usr/bin/env python3
# Standalone replay for 20251203_1514; this file intentionally contains the full BGPy replay engine.
# Event type: leak; prefix: 197.214.32.0/22; root AS: 24691.
# Evidence summary: event-time paths preserve origin AS37531 but insert leak
# AS24691, which exports the learned route to provider AS174.  AS58057 paths in
# the same CSV belong to later path exploration and are not this replay root.

"""Replay one historical BGP anomaly and compare it with observed paths.

The module supports three experiment designs:

* Prefix hijack: inject the legitimate victim route and the attacker route so
  that they compete under the default BGPy Gao--Rexford policy.
...
audited explicitly so a higher score is not produced by hidden test leakage.
"""
...
LeakStageAnchorMode = Literal["off", "calibration"]
PreRibExportTreeMode = Literal["off", "calibration"]
...
def build_pre_rib_export_tree(
    ...
    """Build an event-time downstream export tree from the earlier RIB file.

    Only paths already containing the configured leak AS are eligible.  The
    construction never reads the held-out after-RIB evaluation paths.  This is
    nevertheless event calibration rather than an independent prediction:
    for this event, ``rib_before_incident.csv`` already contains AS24691 leak
    paths and therefore represents an earlier observed event state, not a
    pristine no-anomaly baseline.
    """

    evaluation_receivers = {
        (str(item["prefix"]), int(item["receiver_as"]))
        for item in observed_paths
    }
    prefix_results: dict[str, Any] = {}
    all_exporters: set[int] = set()
    all_targets: set[int] = set()
    all_overlap_receivers: set[int] = set()

    for prefix in prefixes:
        details = leak_details[prefix]
        leak_targets = {
            int(asn) for asn in details.get("event_leak_targets", [])
        }
        exact_rows = [row for row in before_rows if row.prefix == prefix]
        candidate_rows = exact_rows or [
            row
            for row in before_rows
            if is_within_canonical(row.prefix, config.canonical_prefix)
        ]

        downstream_paths: set[tuple[int, ...]] = set()
        for row in candidate_rows:
            normalized = canonicalize_as_path(row.as_path)
            if config.root_asn not in normalized:
                continue
            full_path = tuple(reversed(normalized))
            root_index = full_path.index(config.root_asn)
            downstream = full_path[root_index:]
            if len(downstream) < 2:
                continue
            if leak_targets and int(downstream[1]) not in leak_targets:
                continue
            if len(set(downstream)) != len(downstream):
                continue
            downstream_paths.add(tuple(int(asn) for asn in downstream))

        targets_by_exporter: dict[int, set[int]] = {}
        root_path_by_exporter: dict[int, tuple[int, ...]] = {}
        for path in sorted(downstream_paths, key=lambda value: (len(value), value)):
            # AS24691's illegal first export is already controlled by
            # event_leak_targets.  The historical tree starts at its calibrated
            # first-hop AS and constrains each subsequent export separately.
            for index in range(1, len(path) - 1):
                exporter = int(path[index])
                target = int(path[index + 1])
                targets_by_exporter.setdefault(exporter, set()).add(target)
                candidate_root_path = tuple(path[: index + 1])
                current = root_path_by_exporter.get(exporter)
                if current is None or (len(candidate_root_path), candidate_root_path) < (
                    len(current),
                    current,
                ):
                    root_path_by_exporter[exporter] = candidate_root_path

        path_receivers = {int(path[-1]) for path in downstream_paths}
        overlap_receivers = sorted(
            receiver
            for receiver in path_receivers
            if (prefix, receiver) in evaluation_receivers
        )
        all_exporters.update(targets_by_exporter)
        all_targets.update(
            target
            for targets in targets_by_exporter.values()
            for target in targets
        )
        all_overlap_receivers.update(overlap_receivers)
        prefix_results[prefix] = {
            "source": "rib_before_incident.csv",
            "source_is_clean_pre_event_baseline": False,
            "source_note": (
                "The file already contains paths through the leak AS; this "
                "tree is an event-calibrated historical export envelope."
            ),
            "candidate_row_scope": "exact_prefix" if exact_rows else "canonical_scope_fallback",
            "path_count": len(downstream_paths),
            "paths": [list(path) for path in sorted(downstream_paths)],
            "targets_by_exporter": {
                str(asn): sorted(targets)
                for asn, targets in sorted(targets_by_exporter.items())
            },
            "root_path_by_exporter": {
                str(asn): list(path)
                for asn, path in sorted(root_path_by_exporter.items())
            },
            "evaluation_receiver_overlap": overlap_receivers,
            "evaluation_paths_used_for_construction": False,
        }
    ...
    return {
        ...
        "source": "rib_before_incident.csv",
        "source_is_clean_pre_event_baseline": False,
        "event_calibrated_internal_consistency_only": True,
        "prefixes": prefix_results,
        "exporters": sorted(all_exporters),
        "targets": sorted(all_targets),
        "evaluation_receiver_overlap": sorted(all_overlap_receivers),
        "evaluation_paths_used_for_construction": False,
    }
...
def resolve_event_inputs(
    ...
    *,
    ...
    if config.anomaly_type == "hijack":
        ...
        resolved["route_leak_pre_rib_export_tree"] = build_pre_rib_export_tree(
            config,
            before_rows,
            prefixes,
            leak_details,
        ...
        resolved["route_leak_calibration_audit"] = {
            "affected_prefixes": sorted(leak_details),
            "calibration_export_targets": sorted(
                {
                    int(asn)
                    for details in leak_details.values()
                    for asn in details["event_leak_targets"]
                }
            ),
            "calibration_path_count": len(calibration_paths),
            "evaluation_path_count": len(observed_paths),
            "calibration_evaluation_overlap": split_audit.get(
    ...
    *,
    ...
    class CAIDAASGraphConstructor(BGPyCAIDAASGraphConstructor):
        ...
        def _get_as_graph_info(
            ...
            if not removed and not added_edges:
                return info
...
def build_announcements(
    ...
    *,
    ...
    leak_stage_anchor_mode: LeakStageAnchorMode = "calibration",
    pre_rib_export_tree_mode: PreRibExportTreeMode = "calibration",
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix, details in sorted(resolved["route_leak"].items()):
            ...
            if leak_stage_anchor_mode == "off":
                continue

            # AS24691 exported the route to AS174 in every event-time path.
            # Seed the calibrated route at the observed first hop so the
            # simplified global legal-route propagation cannot erase that
            # event-time local-policy decision before AS174 exports it.
            for target_asn in details["event_leak_targets"]:
                target_asn = int(target_asn)
                target_obj = as_graph.as_dict.get(target_asn)
                if target_obj is None:
                    audit["warnings"].append(
                        f"Calibration leak target AS{target_asn} is absent "
                        f"from the CAIDA graph for {prefix}; stage anchor skipped."
                    )
                    continue
                target_recv_relationship = relationship_enum(
                    target_obj, config.root_asn
                )
                target_relationship_source = "caida"
                if target_recv_relationship is None:
                    target_recv_relationship = Relationships.PEERS
                    target_relationship_source = "fallback_peer"
                    audit["warnings"].append(
                        f"AS{target_asn} is not adjacent to leak AS"
                        f"{config.root_asn} in CAIDA for {prefix}; stage "
                        "anchor recv_relationship defaults to peer."
                    )

                stage_path = (target_asn, *anchor)
                stage_announcement = Announcement(
                    prefix=prefix,
                    as_path=stage_path,
                    seed_asn=target_asn,
                    next_hop_asn=target_asn,
                    recv_relationship=target_recv_relationship,
                    timestamp=Timestamps.ATTACKER.value,
                )
                announcements.append(stage_announcement)
                audit["announcements"].append(
                    {
                        "prefix": prefix,
                        "seed_as": target_asn,
                        "type": "calibration_leak_export_stage_anchor",
                        "as_path_view": list(stage_path),
                        "received_relationship": (
                            target_recv_relationship.name.lower()
                        ),
                        "relationship_source": target_relationship_source,
                        "calibration_only": True,
                        "evaluation_paths_used": False,
                    }
                )

            if pre_rib_export_tree_mode == "calibration":
                tree_prefix = resolved.get(
                    "route_leak_pre_rib_export_tree", {}
                ).get("prefixes", {}).get(prefix, {})
                target_stage_asns = {
                    int(asn) for asn in details["event_leak_targets"]
                }
                for exporter_raw, root_path_raw in sorted(
                    tree_prefix.get("root_path_by_exporter", {}).items(),
                    key=lambda item: int(item[0]),
                ):
                    exporter_asn = int(exporter_raw)
                    # The first-hop stage anchor was already added above.
                    if exporter_asn in target_stage_asns:
                        continue
                    root_path = tuple(int(asn) for asn in root_path_raw)
                    if (
                        len(root_path) < 3
                        or root_path[0] != config.root_asn
                        or root_path[-1] != exporter_asn
                    ):
                        audit["warnings"].append(
                            f"Invalid pre-RIB export-tree root path for AS"
                            f"{exporter_asn} and {prefix}: {list(root_path)}"
                        )
                        continue
                    exporter_obj = as_graph.as_dict.get(exporter_asn)
                    if exporter_obj is None:
                        audit["warnings"].append(
                            f"Pre-RIB export-tree AS{exporter_asn} is absent "
                            f"from the CAIDA graph for {prefix}; anchor skipped."
                        )
                        continue
                    previous_asn = int(root_path[-2])
                    exporter_recv_relationship = relationship_enum(
                        exporter_obj, previous_asn
                    )
                    exporter_relationship_source = "caida"
                    if exporter_recv_relationship is None:
                        exporter_recv_relationship = Relationships.PEERS
                        exporter_relationship_source = "fallback_peer"
                        audit["warnings"].append(
                            f"AS{exporter_asn} is not adjacent to previous "
                            f"exporter AS{previous_asn} in CAIDA for {prefix}; "
                            "pre-RIB tree recv_relationship defaults to peer."
                        )

                    # Announcement.as_path is receiver-to-origin.  The tree
                    # stores root-to-exporter paths, while anchor[1:] preserves
                    # the route AS24691 originally learned toward AS37531.
                    exporter_path = tuple(reversed(root_path)) + anchor[1:]
                    exporter_announcement = Announcement(
                        prefix=prefix,
                        as_path=exporter_path,
                        seed_asn=exporter_asn,
                        next_hop_asn=exporter_asn,
                        recv_relationship=exporter_recv_relationship,
                        timestamp=Timestamps.ATTACKER.value,
                    )
                    announcements.append(exporter_announcement)
                    audit["announcements"].append(
                        {
                            "prefix": prefix,
                            "seed_as": exporter_asn,
                            "type": "pre_rib_export_tree_stage_anchor",
                            "as_path_view": list(exporter_path),
                            "root_to_exporter_path": list(root_path),
                            "forced_export_targets": tree_prefix.get(
                                "targets_by_exporter", {}
                            ).get(str(exporter_asn), []),
                            "received_relationship": (
                                exporter_recv_relationship.name.lower()
                            ),
                            "relationship_source": exporter_relationship_source,
                            "source": "rib_before_incident.csv",
                            "event_calibrated": True,
                            "evaluation_paths_used": False,
                        }
                    )

    audit["pre_rib_export_tree_mode"] = pre_rib_export_tree_mode
...
def make_calibration_stage_export_policy(
    *,
    root_asn: int,
    forced_announcements_by_asn: dict[int, tuple[Any, ...]],
    forced_targets_by_asn_prefix: dict[
        int, dict[str, frozenset[int] | None]
    ],
) -> type[Any]:
    """Export calibrated leak-stage routes even if they lose Local-RIB.

    A stage announcement alone can be replaced by the globally visible legal
    route before the first-hop AS propagates it.  The historical RIB proves
    that the calibrated first hop did export the leak.  This policy preserves
    ordinary BGP propagation and additionally sends only those calibration
    stage announcements that contain the configured leak AS.
    """

    from bgpy.shared.enums import Relationships
    from bgpy.simulation_engine import BGPFullIgnoreInvalid

    class CalibrationStageExportPolicy(BGPFullIgnoreInvalid):
        name = "Calibration Leak-Stage Export BGP"
        event_root_asn = int(root_asn)
        event_announcements_by_asn = forced_announcements_by_asn
        event_targets_by_asn_prefix = forced_targets_by_asn_prefix

        def _propagate_forced_stage(
            self,
            propagate_to: Any,
            send_rels: set[Any],
        ) -> None:
            if propagate_to == Relationships.PROVIDERS:
                ...
                raise NotImplementedError(propagate_to)

            forced = self.event_announcements_by_asn.get(
                int(self.as_.asn), ()
            )
            for unprocessed_ann in forced:
                if self.event_root_asn not in unprocessed_ann.as_path:
                    continue
                ann = unprocessed_ann.copy(
                    {"next_hop_asn": int(self.as_.asn)}
                )
                targets = self.event_targets_by_asn_prefix.get(
                    int(self.as_.asn), {}
                ).get(ann.prefix)
                for neighbor in neighbors:
                    if targets is not None and int(neighbor.asn) not in targets:
                        continue
                    if int(neighbor.asn) in ann.as_path:
                        continue
        ...
        def propagate_to_providers(self) -> None:
            ...
            self._propagate_forced_stage(
                Relationships.PROVIDERS,
                {Relationships.ORIGIN, Relationships.CUSTOMERS},
            )

        def propagate_to_peers(self) -> None:
            super().propagate_to_peers()
            self._propagate_forced_stage(
                Relationships.PEERS,
                {Relationships.ORIGIN, Relationships.CUSTOMERS},
            )
    ...
    return CalibrationStageExportPolicy
...
def run_simulation(
    ...
    *,
    ...
    leak_stage_anchor_mode: LeakStageAnchorMode,
    pre_rib_export_tree_mode: PreRibExportTreeMode,
    legal_route_mode: LegalRouteMode,
    path_source: PathSourceMode,
) -> tuple[Any, Any, dict[str, Any]]:
    from bgpy.shared.enums import Relationships
    from bgpy.simulation_engine import BGP, BGPFullIgnoreInvalid, SimulationEngine
    ...
    announcements, announcement_audit = build_announcements(
        ...
        resolved,
        as_graph,
        legal_route_mode=legal_route_mode,
        leak_stage_anchor_mode=leak_stage_anchor_mode,
        pre_rib_export_tree_mode=pre_rib_export_tree_mode,
    )
    ...
    stage_target_asns = frozenset(
        int(asn)
        for asn in resolved.get("route_leak_calibration_audit", {}).get(
            "calibration_export_targets", []
        )
    )
    pre_rib_tree = resolved.get("route_leak_pre_rib_export_tree", {})
    pre_rib_exporters = (
        frozenset(int(asn) for asn in pre_rib_tree.get("exporters", []))
        if pre_rib_export_tree_mode == "calibration"
        else frozenset()
    )
    stage_target_asns = stage_target_asns | pre_rib_exporters
    forced_stage_announcements: dict[int, tuple[Any, ...]] = {
        target_asn: tuple(
            ann
            for ann in announcements
            if int(getattr(ann, "seed_asn", -1)) == target_asn
            and config.root_asn in ann.as_path
        )
        for target_asn in stage_target_asns
    }
    forced_stage_announcements = {
        asn: anns
        for asn, anns in forced_stage_announcements.items()
        if anns
    }
    forced_targets_by_asn_prefix: dict[
        int, dict[str, frozenset[int] | None]
    ...
    if pre_rib_export_tree_mode == "calibration":
        for prefix, tree_details in pre_rib_tree.get("prefixes", {}).items():
            for exporter_raw, targets in tree_details.get(
                "targets_by_exporter", {}
            ).items():
                forced_targets_by_asn_prefix.setdefault(
                    int(exporter_raw), {}
                )[str(prefix)] = frozenset(int(asn) for asn in targets)
    # With the tree disabled, preserve the earlier first-hop intervention:
    # calibrated AS174 may export according to its ordinary relationship
    # methods rather than an exact historical downstream target set.
    for asn in forced_stage_announcements:
        forced_targets_by_asn_prefix.setdefault(asn, {})
    stage_export_policy = (
        make_calibration_stage_export_policy(
            root_asn=config.root_asn,
            forced_announcements_by_asn=forced_stage_announcements,
            forced_targets_by_asn_prefix=forced_targets_by_asn_prefix,
        )
        if config.anomaly_type == "leak"
        and leak_stage_anchor_mode == "calibration"
        and forced_stage_announcements
        else base_policy
    )
    adopting_asns = (
        frozenset(forced_stage_announcements)
        if stage_export_policy is not base_policy
        else frozenset()
    )
    scenario_config = ScenarioConfig(
        ScenarioCls=Scenario,
        BasePolicyCls=base_policy,
        AdoptPolicyCls=stage_export_policy,
        ...
        override_adopting_asns=adopting_asns,
    ...
    announcement_audit["path_retention_policy"] = {
        "requested_path_source": path_source,
        "base_policy": base_policy.__name__,
        "ribs_in_retained": path_source == "received-envelope",
    }
    announcement_audit["calibration_stage_anchors"] = {
        "enabled": (
            config.anomaly_type == "leak"
            and leak_stage_anchor_mode == "calibration"
        ),
        "mode": leak_stage_anchor_mode,
        "targets": resolved.get("route_leak_calibration_audit", {}).get(
            "calibration_export_targets", []
        ),
        "evaluation_paths_used": False,
        "forced_export_policy_enabled": bool(adopting_asns),
        "forced_export_asns": sorted(adopting_asns),
        "forced_announcement_count": sum(
            len(anns) for anns in forced_stage_announcements.values()
        ),
    }
    announcement_audit["pre_rib_export_tree"] = {
        "enabled": (
            config.anomaly_type == "leak"
            and leak_stage_anchor_mode == "calibration"
            and pre_rib_export_tree_mode == "calibration"
            and bool(pre_rib_exporters)
        ),
        "mode": pre_rib_export_tree_mode,
        "source": pre_rib_tree.get("source"),
        "source_is_clean_pre_event_baseline": pre_rib_tree.get(
            "source_is_clean_pre_event_baseline"
        ),
        "event_calibrated_internal_consistency_only": pre_rib_tree.get(
            "event_calibrated_internal_consistency_only", False
        ),
        "exporters": sorted(pre_rib_exporters),
        "evaluation_receiver_overlap": pre_rib_tree.get(
            "evaluation_receiver_overlap", []
        ),
        "evaluation_paths_used_for_construction": False,
    }
...
def compare_with_observed_paths(
    ...
    leak_calibration_targets_used = (
        resolved["metadata"]["anomaly_type"] == "leak"
        and simulation.get("leak_target_mode") == "event"
        and bool(
            resolved.get("route_leak_calibration_audit", {}).get(
                "calibration_export_targets", []
            )
        )
    )
    leak_stage_anchors_used = bool(
        simulation.get("calibration_stage_anchors_used", False)
    )
    leak_stage_forced_export_used = bool(
        simulation.get(
            "calibration_stage_forced_export_policy_used", False
        )
    )
    pre_rib_export_tree_used = bool(
        simulation.get("pre_rib_export_tree_used", False)
    )
    ...
    if leak_calibration_targets_used:
        quality_flags.append("event_observed_leak_targets_used")
    if leak_stage_anchors_used:
        quality_flags.append("event_calibration_stage_anchors_used")
    if leak_stage_forced_export_used:
        quality_flags.append("event_calibration_stage_forced_export_used")
    if pre_rib_export_tree_used:
        quality_flags.append("event_pre_rib_export_tree_used")
        quality_flags.append("event_calibrated_internal_consistency_only")
    if (
        (leak_calibration_targets_used or leak_stage_anchors_used)
        and split_audit.get("calibration_evaluation_overlap", True)
    ):
        quality_flags.append("event_evaluation_paths_used_for_leak_constraints")
    ...
    return {
        ...
        "evaluation_quality": {
            ...
            "event_observed_leak_targets_used": leak_calibration_targets_used,
            "event_calibration_stage_anchors_used": leak_stage_anchors_used,
            "event_calibration_stage_forced_export_used": (
                leak_stage_forced_export_used
            ),
            "event_pre_rib_export_tree_used": pre_rib_export_tree_used,
            "event_calibrated_internal_consistency_only": (
                pre_rib_export_tree_used
            ),
            "leak_constraint_evaluation_overlap": (
                (leak_calibration_targets_used or leak_stage_anchors_used)
                and split_audit.get("calibration_evaluation_overlap", True)
            ),
            "validation_split": split_audit,
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=2,
        help=(
            "Minimum receiver/prefix pairs required for a holdout split. "
            "This six-path event defaults to 2 so calibration and evaluation "
            "remain disjoint instead of falling back to complete overlap."
        ),
    )
    parser.add_argument(
        "--topology-overlay-mode",
        choices=("none", "pre-rib", "pre-rib-and-calibration"),
        default="pre-rib-and-calibration",
        help=(
            "Add RIB-observed missing edges only to the per-event in-memory graph. "
            "The cached CAIDA topology is never modified."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "retained in every AS's RIBs-In. Default: received-envelope."
        ),
    )
    parser.add_argument(
        "--leak-target-mode",
        ...
        help=(
            ...
            "Default for this event: event."
        ),
    )
    parser.add_argument(
        "--leak-stage-anchor-mode",
        choices=("off", "calibration"),
        default="calibration",
        help=(
            "Seed the leaked route at calibration-observed first-hop ASes, or "
            "disable the intervention for ablation. Default: calibration."
        ),
    )
    parser.add_argument(
        ...
        default="calibration",
        help=(
            "Use the downstream export tree observed in rib_before_incident.csv "
            "to constrain calibrated stage exports, or disable it for ablation. "
            "For this event the file already contains leak paths, so the default "
            "is an event-calibrated internal-consistency experiment."
        ),
    )
    parser.add_argument(
        "--legal-route-mode",
        ...
        help=(
            ...
            "rib_after_incident.csv, or never. Default: after-observed."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def run_cli(config: EventConfig) -> None:
    ...
    engine, _scenario, announcement_audit = run_simulation(
        ...
        leak_stage_anchor_mode=args.leak_stage_anchor_mode,
        pre_rib_export_tree_mode=args.pre_rib_export_tree_mode,
    ...
    simulation["leak_stage_anchor_mode"] = (
        args.leak_stage_anchor_mode
        if config.anomaly_type == "leak"
        else "not_applicable"
    )
    simulation["pre_rib_export_tree_mode"] = (
        args.pre_rib_export_tree_mode
        if config.anomaly_type == "leak"
        else "not_applicable"
    )
    simulation["calibration_stage_anchors_used"] = (
        config.anomaly_type == "leak"
        and args.leak_stage_anchor_mode == "calibration"
        and bool(
            resolved.get("route_leak_calibration_audit", {}).get(
                "calibration_export_targets", []
            )
    ...
    simulation["calibration_stage_forced_export_policy_used"] = bool(
        announcement_audit.get("calibration_stage_anchors", {}).get(
            "forced_export_policy_enabled", False
        )
    )
    simulation["pre_rib_export_tree_used"] = bool(
        announcement_audit.get("pre_rib_export_tree", {}).get(
            "enabled", False
        )
    )
    ...
    complete_result = {
        "metadata": {
            **resolved["metadata"],
            ...
            "leak_stage_anchor_mode": (
                args.leak_stage_anchor_mode
                if config.anomaly_type == "leak"
                else "not_applicable"
            ),
            "pre_rib_export_tree_mode": (
                args.pre_rib_export_tree_mode
                if config.anomaly_type == "leak"
                else "not_applicable"
            ),
...
CONFIG = EventConfig(
    event_id='20251203_1514',
    event_name='Leak event 20251203_1514',
    anomaly_type='leak',
    canonical_prefix='197.214.32.0/22',
    root_asn=24691,
    victim_asn=None,
    caida_snapshot=datetime(2025, 12, 1),
)
