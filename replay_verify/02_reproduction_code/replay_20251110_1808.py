#!/usr/bin/env python3
# Standalone replay for 20251110_1808; this file intentionally contains the full BGPy replay engine.
# Event type: hijack; prefix: 77.105.167.0/24; root AS: 205784.
# Evidence summary: the RIB transition changes the origin from AS210644 to
# AS205784.  The root-cause report additionally identifies AS52025 export
# leakage, so this replay evaluates the anomalous-origin propagation envelope
# while retaining the composite-event diagnosis in its audit.
...
HijackExportMode = Literal["all", "calibration-first-hop"]
HijackStageAnchorMode = Literal["off", "calibration-relationship"]
...
def build_hijack_calibration_export_tree(
    config: EventConfig,
    prefixes: Sequence[str],
    calibration_paths: Sequence[dict[str, Any]],
) -> dict[str, Any]:
    """Build a propagation-stage tree from disjoint calibration receivers.

    Held-out evaluation paths are never inspected.  The tree records the
    attacker-to-exporter path and the next hops observed below each exporter.
    At simulation time, those next hops are converted to relationship classes
    so the intervention can generalize to unseen receivers without hard-coding
    their paths.
    """
    ...
    for prefix in prefixes:
        paths = sorted(
            {
                tuple(int(asn) for asn in item["propagation_path"])
                for item in calibration_paths
                if item["prefix"] == prefix
                and item["propagation_path"]
                and int(item["propagation_path"][0]) == config.root_asn
            },
        ...
        for path in paths:
            for index in range(1, len(path) - 1):
                ...
                candidate = tuple(path[: index + 1])
                current = root_path_by_exporter.get(exporter)
                if current is None or (len(candidate), candidate) < (
                    len(current),
                    current,
                ):
                    root_path_by_exporter[exporter] = candidate
        all_exporters.update(targets_by_exporter)
        prefix_results[prefix] = {
            "source": "disjoint_after_rib_calibration_partition",
            "calibration_path_count": len(paths),
            "calibration_paths": [list(path) for path in paths],
            "targets_by_exporter": {
...
def resolve_event_inputs(
    ...
    *,
    ...
    if config.anomaly_type == "hijack":
        ...
        calibration_export_targets: dict[str, list[int]] = {}
        for anomalous_prefix in prefixes:
            ...
            calibration_export_targets[anomalous_prefix] = sorted(
                {
                    int(item["propagation_path"][1])
                    for item in calibration_paths
                    if item["prefix"] == anomalous_prefix
                    and len(item["propagation_path"]) > 1
                    and int(item["propagation_path"][0]) == config.root_asn
                }
            )
        resolved["hijack"] = {
            ...
            "composite_event_audit": {
                "primary_replay_component": "origin_change_210644_to_205784",
                "secondary_reported_behavior": "route_leak_by_as52025",
                "reported_abnormal_triplet": [64289, 6461, 28917],
                "note": (
                    "Similarity evaluation selects paths originated by AS205784. "
                    "The AS52025 leak is retained as a secondary event component "
                    "and is not incorrectly substituted for the observed origin."
                ),
            },
            "victim_prefix_by_anomalous_prefix": victim_prefixes,
            "victim_prefix_evidence": victim_evidence,
            "calibration_export_targets_by_prefix": calibration_export_targets,
            "export_target_source": (
                "disjoint calibration receivers only; evaluation paths excluded"
            ),
        ...
        resolved["hijack_calibration_export_tree"] = (
            build_hijack_calibration_export_tree(
                config,
                prefixes,
                calibration_paths,
            )
...
def build_announcements(
    ...
    *,
    ...
    hijack_stage_anchor_mode: HijackStageAnchorMode = "calibration-relationship",
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix in resolved["anomalous_prefixes"]:
            ...
            if hijack_stage_anchor_mode == "calibration-relationship":
                tree_prefix = resolved.get(
                    "hijack_calibration_export_tree", {}
                ...
                for exporter_raw, root_path_raw in sorted(
                    ...
                    if (
                        ...
                        audit["warnings"].append(
                            f"Invalid calibration root path for AS{exporter_asn} "
                            f"and {prefix}: {list(root_path)}"
                        )
                        continue
                    exporter_obj = as_graph.as_dict.get(exporter_asn)
                    if exporter_obj is None:
                        audit["warnings"].append(
                            f"Calibration exporter AS{exporter_asn} is absent "
                            f"from CAIDA for {prefix}; stage anchor skipped."
                        )
                        continue
                    previous_asn = int(root_path[-2])
                    recv_relationship = relationship_enum(
                    ...
                    if recv_relationship is None:
                        ...
                        audit["warnings"].append(
                            f"AS{exporter_asn} is not adjacent to previous AS"
                            f"{previous_asn} in CAIDA for {prefix}; stage "
                            "recv_relationship defaults to peer."
                        )
                    stage_path = tuple(reversed(root_path))
                    ...
                    audit["announcements"].append(
                        {
                            ...
                            "type": "hijack_calibration_stage_anchor",
                            "as_path_view": list(stage_path),
                            "root_to_exporter_path": list(root_path),
                            "calibration_next_hops": tree_prefix.get(
                                "targets_by_exporter", {}
    ...
    audit["hijack_stage_anchor_mode"] = hijack_stage_anchor_mode
...
def make_hijack_export_policy(
    targets_by_prefix: dict[str, frozenset[int] | None],
) -> type[Any]:
    """Restrict attacker exports to first hops seen in calibration only."""

    from bgpy.simulation_engine import BGPFullIgnoreInvalid

    class CalibrationAnchoredHijackPolicy(BGPFullIgnoreInvalid):
        name = "Calibration-Anchored Prefix Hijack"
        event_targets_by_prefix = targets_by_prefix
        ...
        def _policy_propagate(
            ...
            targets = self.event_targets_by_prefix.get(ann.prefix)
            if targets is not None and int(neighbor.asn) not in targets:
                return True
            return False

    return CalibrationAnchoredHijackPolicy


def make_hijack_stage_export_policy(
    *,
    root_asn: int,
    forced_announcements_by_asn: dict[int, tuple[Any, ...]],
    allowed_relationships_by_asn_prefix: dict[
        int, dict[str, frozenset[Any] | None]
    ],
) -> type[Any]:
    """Preserve calibration-stage routes and generalize their export class.

    A stage route can lose to the legitimate AS210644 route before it reaches
    held-out receivers.  Calibration proves that the stage AS selected and
    exported the AS205784 route.  We therefore retain that route and export it
    only to commercial neighbor classes observed below the same exporter in
    calibration; exact evaluation ASNs are never used.
    """

    from bgpy.shared.enums import Relationships
    from bgpy.simulation_engine import BGPFullIgnoreInvalid

    class CalibrationRelationshipStagePolicy(BGPFullIgnoreInvalid):
        name = "Calibration Relationship-Stage Hijack BGP"
        event_root_asn = int(root_asn)
        event_announcements_by_asn = forced_announcements_by_asn
        event_allowed_relationships = allowed_relationships_by_asn_prefix
        ...
        def _propagate_forced_stage(
            ...
            for unprocessed_ann in forced:
                if self.event_root_asn not in unprocessed_ann.as_path:
                    continue
                allowed = self.event_allowed_relationships.get(
                    int(self.as_.asn), {}
                ).get(unprocessed_ann.prefix)
                if allowed is not None and propagate_to not in allowed:
                    continue
        ...
        def propagate_to_providers(self) -> None:
            ...
            self._propagate_forced_stage(Relationships.PROVIDERS)

        def propagate_to_peers(self) -> None:
            super().propagate_to_peers()
            self._propagate_forced_stage(Relationships.PEERS)

        def propagate_to_customers(self) -> None:
            super().propagate_to_customers()
            self._propagate_forced_stage(Relationships.CUSTOMERS)

    return CalibrationRelationshipStagePolicy
...
def validate_topology(
    ...
    if config.anomaly_type == "hijack":
        ...
        export_targets = sorted(
            {
                int(asn)
                for targets in resolved["hijack"][
                    "calibration_export_targets_by_prefix"
                ].values()
                for asn in targets
            }
        )
        topology_audit["hijack_calibration_export_targets"] = export_targets
        topology_audit["hijack_export_target_relationships"] = {
            str(asn): relationship_name(root_obj, asn)
            for asn in export_targets
        }
        topology_audit["hijack_stage_exporters"] = resolved.get(
            "hijack_calibration_export_tree", {}
        ).get("exporters", [])
...
def run_simulation(
    ...
    *,
    ...
    hijack_export_mode: HijackExportMode,
    hijack_stage_anchor_mode: HijackStageAnchorMode,
    ...
    announcements, announcement_audit = build_announcements(
        ...
        hijack_stage_anchor_mode=hijack_stage_anchor_mode,
    )
    attacker_policy = None
    if (
        config.anomaly_type == "hijack"
        and hijack_export_mode == "calibration-first-hop"
    ):
        target_mapping = {
            prefix: (
                frozenset(int(asn) for asn in targets)
                if targets
                else None
            )
            for prefix, targets in resolved["hijack"][
                "calibration_export_targets_by_prefix"
            ].items()
        }
        attacker_policy = make_hijack_export_policy(target_mapping)
        announcement_audit["hijack_export_constraint"] = {
            "mode": hijack_export_mode,
            "targets_by_prefix": {
                prefix: sorted(targets) if targets is not None else None
                for prefix, targets in target_mapping.items()
            },
            "source": "disjoint calibration receiver paths only",
            "evaluation_paths_used": False,
        }
    elif config.anomaly_type == "leak":
        target_mapping = (
    ...
    allowed_stage_relationships: dict[
        int, dict[str, frozenset[Any] | None]
    ] = {}
    relationship_audit: dict[str, dict[str, list[str] | None]] = {}
    if (
        config.anomaly_type == "hijack"
        and hijack_stage_anchor_mode == "calibration-relationship"
    ):
        stage_exporters = {
            int(asn)
            for asn in resolved.get(
                "hijack_calibration_export_tree", {}
            ).get("exporters", [])
        }
        forced_stage_announcements = {
            exporter: tuple(
                ...
                if int(getattr(ann, "seed_asn", -1)) == exporter
                and config.root_asn in ann.as_path
            )
            for exporter in stage_exporters
        }
        forced_stage_announcements = {
            asn: anns
        ...
        tree_prefixes = resolved["hijack_calibration_export_tree"]["prefixes"]
        for prefix, tree_details in tree_prefixes.items():
            for exporter_raw, target_asns in tree_details[
                "targets_by_exporter"
            ].items():
                exporter = int(exporter_raw)
                exporter_obj = as_graph.as_dict.get(exporter)
                if exporter_obj is None:
                    continue
                relationships = frozenset(
                    relationship
                    for target in target_asns
                    for relationship in [
                        relationship_enum(exporter_obj, int(target))
                    ]
                    if relationship is not None
                )
                allowed_stage_relationships.setdefault(exporter, {})[
                    prefix
                ] = relationships or None
                relationship_audit.setdefault(str(exporter), {})[
                    prefix
                ] = (
                    sorted(relationship.name.lower() for relationship in relationships)
                    if relationships
                    else None
                )

    stage_export_policy = (
        make_hijack_stage_export_policy(
            root_asn=config.root_asn,
            forced_announcements_by_asn=forced_stage_announcements,
            allowed_relationships_by_asn_prefix=allowed_stage_relationships,
        )
        if forced_stage_announcements
        else base_policy
    )
    adopting_asns = (
    ...
    announcement_audit["hijack_calibration_stage"] = {
        "enabled": bool(adopting_asns),
        "mode": hijack_stage_anchor_mode,
        ...
        "allowed_relationships_by_asn_prefix": relationship_audit,
        "source": "disjoint calibration receiver paths only",
        "evaluation_paths_used": False,
        "event_calibrated_internal_consistency_only": bool(adopting_asns),
    }
...
def compare_with_observed_paths(
    ...
    hijack_first_hop_anchors_used = (
        resolved["metadata"]["anomaly_type"] == "hijack"
        and simulation.get("hijack_export_mode") == "calibration-first-hop"
        and any(
            resolved.get("hijack", {})
            .get("calibration_export_targets_by_prefix", {})
            .values()
        )
    )
    hijack_stage_anchors_used = bool(
        simulation.get("hijack_calibration_stage_used", False)
    )
    ...
    if hijack_first_hop_anchors_used:
        quality_flags.append("event_observed_first_hop_anchors_used")
    if hijack_stage_anchors_used:
        quality_flags.append("event_calibration_stage_anchors_used")
        quality_flags.append("event_calibration_relationship_export_used")
    ...
    return {
        ...
        "evaluation_quality": {
            ...
            "event_observed_first_hop_anchors_used": (
                hijack_first_hop_anchors_used
            ),
            "event_calibration_stage_anchors_used": hijack_stage_anchors_used,
            "event_calibration_relationship_export_used": (
                hijack_stage_anchors_used
            ),
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--hijack-export-mode",
        choices=("all", "calibration-first-hop"),
        default="calibration-first-hop",
        help=(
            "Export the hijack to every CAIDA neighbor or only to first hops "
            "seen in the disjoint calibration paths. Default: calibration-first-hop."
        ),
    )
    parser.add_argument(
        "--hijack-stage-anchor-mode",
        choices=("off", "calibration-relationship"),
        default="calibration-relationship",
        help=(
            "Retain routes at calibration-observed propagation stages and "
            "generalize export only to the same neighbor relationship classes. "
            "Use off for the independent-stage ablation."
        ),
    )
    parser.add_argument(
        "--legal-route-mode",
...
def run_cli(config: EventConfig) -> None:
    ...
    engine, _scenario, announcement_audit = run_simulation(
        ...
        hijack_export_mode=args.hijack_export_mode,
        hijack_stage_anchor_mode=args.hijack_stage_anchor_mode,
    ...
    simulation["hijack_export_mode"] = (
        args.hijack_export_mode
        if config.anomaly_type == "hijack"
        else "not_applicable"
    )
    simulation["hijack_stage_anchor_mode"] = (
        args.hijack_stage_anchor_mode
        if config.anomaly_type == "hijack"
        else "not_applicable"
    )
    simulation["hijack_calibration_stage_used"] = bool(
        announcement_audit.get("hijack_calibration_stage", {}).get(
            "enabled", False
        )
    )
...
CONFIG = EventConfig(
    event_id='20251110_1808',
    event_name='Hijack event 20251110_1808',
    anomaly_type='hijack',
    canonical_prefix='77.105.167.0/24',
    root_asn=205784,
    victim_asn=210644,
    caida_snapshot=datetime(2025, 11, 1),
)
