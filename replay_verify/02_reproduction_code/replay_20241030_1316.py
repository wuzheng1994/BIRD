#!/usr/bin/env python3
# Standalone replay for 20241030_1316; this file intentionally contains the full BGPy replay engine.
# Event type: leak; prefix: 108.171.108.0/23; root AS: 49981.
# Evidence: AS140952 -> AS137409 -> AS49981, then leaked to providers/peers.
...
* Route leak: anchor the route learned by the leaking AS, allow the policy
  ...
  event envelope.  This event's primary replay does not separately seed the
  legitimate origin because doing so replaces the event anchor before the
  leak-export round; a separate ``--legal-route-mode after-observed`` run is
  retained as the converged competition counterfactual.
...
# The independent 2024 event implementation and the selected CAIDA snapshot
# both identify AS137409 as a peer of AS49981.  Encoding this directly prevents
# an independently seeded legal announcement from changing the event anchor's
# received class before the route-leak export round.
EVENT_ANCHOR_RECV_RELATIONSHIP = "peer"
REFERENCE_EVENT_FIRST_HOPS = frozenset({1764, 5511})
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix, details in leak_details.items():
            ...
            details["legal_route_evidence_semantics"] = (
                "Rows containing the legitimate origin but excluding paths "
                "that also contain the leak AS"
            )
    ...
    *,
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix, details in sorted(resolved["route_leak"].items()):
            ...
            caida_relationship = relationship_enum(root_obj, origin_neighbor)
            recv_relationship = {
                "customer": Relationships.CUSTOMERS,
                "peer": Relationships.PEERS,
                "provider": Relationships.PROVIDERS,
            }[EVENT_ANCHOR_RECV_RELATIONSHIP]
            relationship_source = "event_observed_peer_anchor"
            if caida_relationship is None:
                audit["warnings"].append(
                    f"AS{origin_neighbor} is not adjacent to AS{config.root_asn} "
                    f"in CAIDA for {prefix}; the event-observed peer anchor is used."
                )
            elif caida_relationship != recv_relationship:
                audit["warnings"].append(
                    f"CAIDA classifies AS{config.root_asn}--AS{origin_neighbor} "
                    f"as {caida_relationship.name.lower()} for {prefix}; the "
                    "event-observed peer anchor takes precedence for replay."
                )
            ...
            audit["announcements"].append(
                {
                    ...
                    "caida_received_relationship": (
                        caida_relationship.name.lower()
                        if caida_relationship is not None
                        else None
                    ),
                }
            )
...
def compare_with_observed_paths(
    ...
    quality_flags.append("event_observed_anchor_relationship_used")
    ...
    return {
        ...
        "evaluation_quality": {
            ...
            "event_observed_anchor_relationship_used": True,
            "anchor_received_relationship": EVENT_ANCHOR_RECV_RELATIONSHIP,
            "reference_event_first_hops": sorted(REFERENCE_EVENT_FIRST_HOPS),
            "validation_split": split_audit,
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "rib_after_incident.csv, or never. Default: withdrawn, matching the "
            "validated event-anchored route-leak experiment. Use after-observed "
            "for a separate converged legal-route competition run."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def run_cli(config: EventConfig) -> None:
    ...
    replay_state_model = (
        "event_anchored_route_leak_envelope"
    ...
    comparison["evaluation_quality"]["state_model_note"] = (
        "The primary run matches the independently validated 2024 event design: "
        "a peer-learned AS49981 anchor is propagated without a separately seeded "
        "AS140952 route replacing it before the leak-export round.  Calibration "
        "and evaluation receivers remain disjoint."
    )
...
CONFIG = EventConfig(
    event_id='20241030_1316',
    event_name='Leak event 20241030_1316',
    anomaly_type='leak',
    canonical_prefix='108.171.108.0/23',
    root_asn=49981,
    victim_asn=None,
