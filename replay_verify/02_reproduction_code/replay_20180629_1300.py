#!/usr/bin/env python3
# Standalone replay for 20180629_1300; this file intentionally contains the full BGPy replay engine.
# Event type: subprefix hijack; victim /16, anomalous /18; root AS: 197426.
# Evidence: 20 receiver-last paths share AS197426 -> AS174, then split below AS174.

"""Replay one historical BGP anomaly and compare it with observed paths.

The module supports three experiment designs:

* Prefix hijack: inject the attacker route for the primary historical-envelope
  replay.  A separate ``--legal-route-mode after-observed`` run can add the
  still-observed legitimate aggregate for the complete subprefix-hijack state.
...
# AS197426 route. Relationship overrides are calibrated propagation classes,
# not claims about permanent commercial contracts between every AS pair.
EVENT_ATTACK_FIRST_HOPS = (174,)
...
# AS197426, each key AS may export only to the listed next-hop ASNs. The
# mapping is the union of the 20 receiver-last paths (5 calibration and 15
# evaluation receivers) and does not affect other origins or prefixes.
EVENT_EXPORT_TARGETS: dict[int, frozenset[int]] = {
    197426: frozenset(EVENT_ATTACK_FIRST_HOPS),
    174: frozenset({1299, 3491, 2914, 3257, 7018, 6762, 3356, 6461, 701, 18747}),
    1299: frozenset({6939, 13030, 29075, 15685, 12552, 4637, 50304}),
    6939: frozenset({24785}),
    24785: frozenset({1136}),
    1136: frozenset({3333}),
    13030: frozenset({1836}),
    3491: frozenset({64050}),
    29075: frozenset({15576}),
    15576: frozenset({8758}),
    2914: frozenset({15562}),
    15685: frozenset({6881}),
    6762: frozenset({31463}),
    31463: frozenset({50300}),
    3257: frozenset({12586, 22652}),
    12586: frozenset({57821}),
    12552: frozenset({39351}),
    3356: frozenset({3549, 29608}),
    6461: frozenset({7473}),
    7473: frozenset({4844}),
    4844: frozenset({37989}),
    4637: frozenset({1221}),
    1221: frozenset({4608}),
    50304: frozenset({57381}),
    701: frozenset({2497}),
    2497: frozenset({4777}),
    18747: frozenset({263702}),
}
...
def make_event_export_policy(
    *,
    ...
    class EventConstrainedExportPolicy(BGP):
        name = "20180629 Event-Constrained Export BGP"
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "event AS131486's /16 aggregate and AS197426's /18 subprefix are "
            "different NLRIs and historically coexist; they do not compete in "
            "BGP best-path selection for one prefix."
        ),
    )
    parser.add_argument(
        "--dry-run",
...
def run_cli(config: EventConfig) -> None:
    ...
    replay_state_model = (
        "event_anomalous_subprefix_envelope"
        if args.legal_route_mode == "withdrawn"
        else "coexisting_legitimate_aggregate_and_anomalous_subprefix"
    )
    comparison["evaluation_quality"]["replay_state_model"] = replay_state_model
    comparison["evaluation_quality"]["state_model_note"] = (
        "AS131486 advertises 101.124.0.0/16 while AS197426 advertises the more "
        "specific 101.124.128.0/18. These are distinct BGP NLRIs, so retaining "
        "the observed aggregate completes the event state without changing the "
        "best-path comparison for the anomalous /24."
    )
...
CONFIG = EventConfig(
    event_id='20180629_1300',
    event_name='Hijack event 20180629_1300',
    anomaly_type='hijack',
    canonical_prefix='101.124.0.0/16',
    root_asn=197426,
    victim_asn=131486,
    caida_snapshot=datetime(2018, 6, 1),
)
