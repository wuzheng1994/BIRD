#!/usr/bin/env python3
# Standalone replay for 20251120_2147; this file intentionally contains the full BGPy replay engine.
# Event type: leak; prefix: 103.31.132.0/23; root AS: 58389.
# Evidence summary: AS58389 learned the affected prefixes from origin AS131749
# and exported them to AS174, AS6762, and AS58552 during the event.  The origin
# remains AS131749, so this is modeled as a route leak rather than a hijack.
...
def resolve_event_inputs(
    ...
    *,
    ...
    if config.anomaly_type == "hijack":
        ...
        # The three affected prefixes form one deaggregation family and share
        # the same leak AS and origin.  A target seen for any sibling prefix in
        # the disjoint calibration partition is therefore valid calibration
        # evidence for the whole family.  This avoids losing AS174 on one /24
        # or AS58552 on the /23 merely because the receiver hash split is
        # performed per prefix.  Evaluation rows are never consulted here.
        ...
        for details in leak_details.values():
            details["per_prefix_calibration_leak_targets"] = list(
                details["event_leak_targets"]
            )
            details["event_leak_targets"] = list(family_targets)
            details["event_target_constraint_available"] = bool(family_targets)
            details["event_target_scope"] = (
                "affected-prefix-family calibration union"
            )
            details["event_target_evaluation_paths_used"] = False

        resolved["route_leak"] = leak_details
        resolved["route_leak_family_audit"] = {
            "affected_prefixes": sorted(leak_details),
            "calibration_export_targets": family_targets,
    ...
    *,
    ...
    if config.anomaly_type == "hijack":
        ...
        for prefix, details in sorted(resolved["route_leak"].items()):
            ...
            # Seed the same event route at calibration-observed export
            # neighbors.  This represents the historically observed local
            # policy decision at the first-hop AS: it accepted the leaked
            # route and propagated it even though a globally seeded legal
            # route may also be available in the simplified Gao--Rexford
            # model.  Only calibration-derived neighbors are used.
...
def run_simulation(
    ...
    *,
    ...
    announcement_audit["calibration_stage_anchors"] = {
        ...
        "targets": resolved.get("route_leak_family_audit", {}).get(
...
def compare_with_observed_paths(
    ...
    leak_calibration_targets_used = (
        ...
        and bool(
            resolved.get("route_leak_family_audit", {}).get(
                "calibration_export_targets", []
            )
        )
    )
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "For this event, event uses the calibration-target union across "
            "the three affected sibling prefixes. Default: event."
        ),
    )
    parser.add_argument(
        ...
        default="calibration",
        help=(
            "Seed the calibrated leaked route at its observed first-hop "
            "neighbors so their event-time local policy choice is represented, "
            "or disable these stage anchors for ablation. Default: calibration."
        ),
    )
    parser.add_argument(
        "--legal-route-mode",
...
CONFIG = EventConfig(
    event_id='20251120_2147',
    event_name='Leak event 20251120_2147',
    anomaly_type='leak',
    canonical_prefix='103.31.132.0/23',
    root_asn=58389,
    victim_asn=None,
