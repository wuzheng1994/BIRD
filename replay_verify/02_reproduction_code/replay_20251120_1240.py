#!/usr/bin/env python3
# Standalone replay for 20251120_1240; this file intentionally contains the full BGPy replay engine.
# Event type: hijack; prefix: 113.20.40.0/22; root AS: 7029.
# Evidence summary: AS 7029 hijacked prefix 113.20.40.0/22. The legitimate origin is AS 397373, but AS 7029 is announcing this prefix. Analysis of the highest-scored path changes shows the origin AS changed from 397373 to 7029 in multiple observed paths after the anomaly event.
    ...
    if config.anomaly_type == "hijack":
        ...
        resolved["hijack"] = {
            ...
            "export_target_source": (
                "calibration receivers only; evaluation receiver paths are excluded"
            ),
...
def make_hijack_export_policy(
    ...
    """Restrict the attacker's exports to calibration-observed first hops.

    The policy is installed only on the configured attacker AS.  Every other
    AS continues to apply ordinary Gao--Rexford export policy.  A ``None``
    target set leaves a prefix unrestricted, which keeps the replay usable
    when the calibration split contains no first-hop evidence.
    """
...
def validate_topology(
    ...
    if config.anomaly_type == "hijack":
        ...
        topology_audit["hijack_export_targets_not_adjacent"] = [
            asn
            for asn in export_targets
            if relationship_name(root_obj, asn) == "not-neighbor"
        ]
...
def run_simulation(
    ...
    *,
    ...
    if config.anomaly_type == "hijack" and hijack_export_mode == "calibration-first-hop":
        target_mapping = {
            prefix: (
        ...
        announcement_audit["hijack_export_constraint"] = {
            ...
            "source": "calibration receiver paths only",
...
def compare_with_observed_paths(
    ...
    if hijack_first_hop_anchors_used:
        ...
        if split_audit.get("calibration_evaluation_overlap", True):
            quality_flags.append(
                "event_evaluation_paths_used_for_first_hop_anchors"
            )
    ...
    return {
        ...
        "evaluation_quality": {
            ...
            "first_hop_anchor_evaluation_overlap": (
                hijack_first_hop_anchors_used
                and split_audit.get("calibration_evaluation_overlap", True)
            ),
            "validation_split": split_audit,
...
def build_arg_parser(config: EventConfig) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "For prefix hijacks, export to every CAIDA neighbor or only to "
            "first hops inferred from the disjoint calibration receivers. "
            "Default: calibration-first-hop."
        ),
    )
    parser.add_argument(
...
CONFIG = EventConfig(
    event_id='20251120_1240',
    event_name='Hijack event 20251120_1240',
    anomaly_type='hijack',
    canonical_prefix='113.20.40.0/22',
    root_asn=7029,
    victim_asn=397373,
